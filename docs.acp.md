...
This document describes how the OpenAEON ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`openaeon acp` exposes an ACP agent over stdio and forwards prompts to a running
OpenAEON Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `openaeon acp` and speaks ACP messages over stdio.
...
`openaeon acp` resolves the Gateway URL and auth from CLI flags or config:
