...
import { OPENAEONApp } from "./app.ts";
import { ChatState, loadChatHistory } from "./controllers/chat.ts";
import { icons } from "./icons.ts";
import { iconForTab, pathForTab, titleForTab, type Tab, TAB_GROUPS } from "./navigation.ts";
...
function resetChatStateForSessionSwitch(state: AppViewState, sessionKey: string) {
  ...
  (state as unknown as OPENAEONApp).resetToolStream();
  (state as unknown as OPENAEONApp).resetChatScroll();
  state.applySettings({
    ...state.settings,
    sessionKey,
...
export function renderTab(state: AppViewState, tab: Tab) {
  ...
  return html`
    <a
      href=${href}
      class="nav-item ${state.tab === tab ? "active" : ""} aeon-entry-anim"
      style="animation-delay: ${TAB_GROUPS.findIndex((g) => (g.tabs as readonly string[]).includes(tab)) * 50}ms"
  ...
  return html`
    <div class="chat-controls">
      ...
      <button
        ...
        @click=${async () => {
          const app = state as unknown as OPENAEONApp;
