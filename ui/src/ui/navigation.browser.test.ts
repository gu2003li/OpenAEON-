...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/openaeon/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/openaeon");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/openaeon/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENAEON_CONTROL_UI_BASE_PATH__ = "/openaeon";
    const app = mountApp("/openaeon/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/openaeon");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/openaeon/sessions");
  });
