...
extension OnboardingView {
    ...
    func welcomePage() -> some View {
        self.onboardingPage {
            VStack(spacing: 22) {
                ...
                Text("OpenAEON 是一款超好用的开源自动控制助手 AI。")
                    .font(.body)
    ...
    func connectionPage() -> some View {
        self.onboardingPage {
            Text("选择网关")
                .font(.largeTitle.weight(.semibold))
            Text(
                "OpenAEON 需要连接到一个后台运行的网关（Gateway）。\n" +
                    ...
                    if self.state.remoteTransport == .ssh {
                        ...
                        GridRow {
                            Text("项目根目录")
                                .font(.callout.weight(.semibold))
                                .frame(width: labelWidth, alignment: .leading)
                            TextField("/home/you/Projects/openaeon", text: self.$state.remoteProjectRoot)
                                .textFieldStyle(.roundedBorder)
                                .frame(width: fieldWidth)
                        }
                        GridRow {
                            Text("CLI 路径")
                                ...
                                .frame(width: labelWidth, alignment: .leading)
                            TextField(
                                "/Applications/OpenAEON.app/.../openaeon",
    ...
    func permissionsPage() -> some View {
        self.onboardingPage {
            Text("授予权限")
                .font(.largeTitle.weight(.semibold))
            Text("这些 macOS 权限允许 OpenAEON 在此 Mac 上自动执行操作并获取上下文。")
                .font(.body)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 520)
        ...
        self.onboardingPage {
            ...
            self.onboardingCard(spacing: 10) {
                ...
                if let cliStatus {
                    ...
                    Text(
                        """
                        为 OpenAEON 安装用户空间 Node 22+ 运行环境和 CLI（不依赖 Homebrew）。
                        随时可以重新运行以重新安装或更新。
                        """)
                        .font(.footnote)
                        .foregroundStyle(.secondary)
    ...
    func workspacePage() -> some View {
        self.onboardingPage {
            Text("智能体工作区")
                .font(.largeTitle.weight(.semibold))
            Text(
                "OpenAEON runs the agent from a dedicated workspace so it can load `AGENTS.md` " +
                ...
                if self.state.connectionMode == .remote {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Workspace folder")
                            .font(.headline)
                        TextField(
                            AgentWorkspace.displayPath(for: OpenAEONConfigFile.defaultWorkspaceURL()),
                                ...
                                Task {
                                    ...
                                    if saved {
                                        self.workspaceStatus =
                                            "Saved to ~/.openaeon/openaeon.json (agents.defaults.workspace)"
                                    }
                                }
                            }
                            .buttonStyle(.bordered)
    ...
    func onboardingChatPage() -> some View {
        VStack(spacing: 16) {
            ...
            self.onboardingGlassCard(padding: 8) {
                OpenAEONChatView(viewModel: self.onboardingChatModel, style: .onboarding)
            ...
            self.onboardingCard {
                ...
                if self.state.connectionMode == .remote {
                    self.featureRow(
                        title: "远程网关检查清单",
                        subtitle: """
                        在您的网关主机上：安装/更新 `openaeon` 软件包并确保凭据存在（通常位于 `~/.openaeon/credentials/oauth.json`）。然后根据需要重新连接。
                        """,
                        systemImage: "network")
                    Divider()
                        .padding(.vertical, 6)
                ...
                self.featureRow(
                    title: "打开菜单栏面板",
                    subtitle: "点击 OpenAEON 菜单栏图标即可进行快速对话和查看状态。",
                ...
                self.skillsOverview
                Toggle("开机启动 OpenAEON", isOn: self.$state.launchAtLogin)
