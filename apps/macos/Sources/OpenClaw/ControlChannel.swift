...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OpenAEONProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.openaeon", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: OpenAEONKit.AnyCodable]()) {
                $0[$1.key] = OpenAEONKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: OpenAEONProtocol.AnyCodable?) -> [String: OpenAEONProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OpenAEONProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OpenAEONKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OpenAEONProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("openaeon.control.heartbeat")
    static let controlAgentEvent = Notification.Name("openaeon.control.agent")
}
