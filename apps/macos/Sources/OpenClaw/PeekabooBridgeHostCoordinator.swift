...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.openaeon", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: OpenAEONPeekabooBridgeServices?
    private static var openaeonSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("OpenAEON", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = OpenAEONPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.openaeonSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.openaeonSocketPath, privacy: .public)")
    }
...
@MainActor
private final class OpenAEONPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.openaeon.peekaboo")
