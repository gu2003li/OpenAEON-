...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("保存在 ~/.openaeon/openaeon.json 中。生产环境推荐使用 OPENAEON_GATEWAY_PASSWORD。")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "保存至 ~/.openaeon/openaeon.json。等待网关重启…"
        } else {
            self.statusMessage = "保存至 ~/.openaeon/openaeon.json。需要重启网关才能生效。"
        }
