...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.openaeon.mac`) even if the shipped
    ...
    {
        if environment["OPENAEON_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "openaeon.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "openaeon.nixMode") { return true }
