    ...
    private var launchdSection: some View {
        GroupBox("Gateway startup") {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text(
                    "开启时，OpenAEON 不会安装或管理 \(gatewayLaunchdLabel)。" +
                        "它将仅附加到现有独立运行的网关进程。")
                    .font(.caption)
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(spacing: 8) {
                    ...
                    Button("复制示例链接") {
                        ...
                        let url = "openaeon://agent?message=\(encoded)&key=\(key)"
                        NSPasteboard.general.clearContents()
                ...
                GridRow {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Toggle("写入轮转诊断日志 (JSONL)", isOn: self.$diagnosticsFileLogEnabled)
                            ...
                            .help(
                                "在 ~/Library/Logs/OpenAEON/ 下写入本地轮转日志。" +
                                    "仅在主动调试排错时启用。")
            ...
            VStack(alignment: .leading, spacing: 12) {
                VStack(alignment: .leading, spacing: 6) {
                    Text("工程配置仓库根目录")
                        .font(.caption.weight(.semibold))
                    HStack(spacing: 8) {
                        TextField("OpenAEON 项目路径", text: self.$gatewayRootInput)
                        ...
                        Button("恢复初始") {
                            let def = FileManager().homeDirectoryForCurrentUser
                                .appendingPathComponent("Projects/openaeon").path
                    ...
                    GridRow {
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            if let sessionStoreSaveError {
                                ...
                                Text("CLI 会话加载器使用的路径；存储于 ~/.openaeon/openaeon.json 中。")
                                    .font(.footnote)
                                    .foregroundStyle(.secondary)
                            }
                        }
    ...
    private var quickActionsSection: some View {
        GroupBox("快速调试选项") {
            VStack(alignment: .leading, spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("注意：为使隐私许可彻底生效，您可能需要重启 OpenAEON。")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)

                    Button {
                        LaunchdManager.startOpenAEON()
                    } label: {
                        Label("彻底重启 OpenAEON", systemImage: "arrow.counterclockwise")
                    }
                    .buttonStyle(.bordered)
                    .controlSize(.small)
                }
...
extension DebugSettings {
    static func exerciseForTesting() async {
        ...
        view.gatewayRootInput = "/tmp/openaeon"
