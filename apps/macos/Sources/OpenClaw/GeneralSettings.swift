        ...
        ScrollView(.vertical) {
            VStack(alignment: .leading, spacing: 18) {
                VStack(alignment: .leading, spacing: 12) {
                    SettingsToggleRow(
                        title: "启用 OpenAEON 网关",
                        subtitle: "暂停以停止 OpenAEON 网关；将不再处理任何新消息。",
                    ...
                    SettingsToggleRow(
                        title: "开机自启动",
                        subtitle: "在您登录 Mac 后自动启动 OpenAEON。",
                        binding: self.$state.launchAtLogin)

                    SettingsToggleRow(
                        title: "显示程序坞(Dock)图标",
                        subtitle: "在底部的 Dock 栏中保持显示 OpenAEON，而不仅仅是顶部菜单栏模式。",
                ...
                HStack {
                    Spacer()
                    Button("彻底关闭 OpenAEON") { NSApp.terminate(nil) }
                ...
                DisclosureGroup(isExpanded: self.$showRemoteAdvanced) {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        LabeledContent("CLI path") {
                            TextField("/Applications/OpenAEON.app/.../openaeon", text: self.$state.remoteCliPath)
                                .textFieldStyle(.roundedBorder)
                                .frame(width: 280)
                        }
                    }
...
extension GeneralSettings {
    ...
    private func revealLogs() {
        ...
        Looked for openaeon logs in /tmp/openaeon/.
    ...
    private func applyDiscoveredGateway(_ gateway: GatewayDiscoveryModel.DiscoveredGateway) {
        ...
        if let endpoint = GatewayDiscoveryHelpers.serviceEndpoint(for: gateway) {
            OpenAEONConfigFile.setRemoteGatewayUrl(
                host: endpoint.host,
                port: endpoint.port)
        } else {
...
extension GeneralSettings {
    static func exerciseForTesting() {
        ...
        state.remoteProjectRoot = "/tmp/openaeon"
        state.remoteCliPath = "/tmp/openaeon"
