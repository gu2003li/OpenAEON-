...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["OPENAEON_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/openaeon")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("openaeon-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("openaeon-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/openaeon/ (rolling or stdout), or nil if none exist.
