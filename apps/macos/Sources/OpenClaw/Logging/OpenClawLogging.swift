...
enum OpenAEONLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = OpenAEONOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OpenAEONFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OpenAEONLogging.bootstrapIfNeeded()
        let label = OpenAEONLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OpenAEONOSLogHandler: LogHandler {
...
struct OpenAEONFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OpenAEONLogging.parseLabel(self.label)
