...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.openaeon", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous OpenAEON run already has an SSH listener on the expected port (common after restarts),
