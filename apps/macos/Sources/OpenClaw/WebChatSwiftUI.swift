...
private let webChatSwiftLogger = Logger(subsystem: "ai.openaeon", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: OpenAEONChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> OpenAEONChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(OpenAEONChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> OpenAEONChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                OpenAEONGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    OpenAEONAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<OpenAEONChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any OpenAEONChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = OpenAEONChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: OpenAEONChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<OpenAEONChatView>) -> NSViewController
    {
