...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.openaeon", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OpenAEONSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OpenAEONSessionPreviewEntry,
    ...
    private static func cache(payload: OpenAEONSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OpenAEONChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OpenAEONKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OpenAEONKit.AnyCodable]) -> [OpenAEONChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OpenAEONChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OpenAEONChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OpenAEONChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OpenAEONChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OpenAEONChatMessage) -> String? {
