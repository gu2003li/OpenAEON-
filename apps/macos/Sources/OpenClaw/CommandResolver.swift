...
enum CommandResolver {
    private static let projectRootDefaultsKey = "openaeon.gatewayProjectRootPath"
    private static let helperName = "openaeon"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let openaeonEntry = root.appendingPathComponent("openaeon.mjs").path
        if FileManager().isReadableFile(atPath: openaeonEntry) { return openaeonEntry }
        let binEntry = root.appendingPathComponent("bin/openaeon.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/openaeon")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let openaeonPaths = self.openaeonManagedPaths(home: home)
        if !openaeonPaths.isEmpty {
            extras.insert(contentsOf: openaeonPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + openaeonPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func openaeonManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".openaeon"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func openaeonExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectOpenAEONExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("openaeon.mjs").path,
            root.appendingPathComponent("bin/openaeon.js").path,
        ]
    ...
    static func hasAnyOpenAEONInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.openaeonExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func openaeonNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let openaeonPath = self.projectOpenAEONExecutable(projectRoot: root) {
            return [openaeonPath, subcommand] + extraArgs
        }
        if let openaeonPath = self.openaeonExecutable(searchPaths: searchPaths) {
            return [openaeonPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "openaeon", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            openaeon entrypoint missing (looked for dist/index.js or openaeon.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real openaeon CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/openaeon"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/openaeon.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/openaeon.mjs"
            node "$PRJ/openaeon.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/openaeon.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/openaeon.js"
            node "$PRJ/bin/openaeon.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent openaeon"
          pnpm --silent openaeon \(quotedArgs);
        else
          echo "openaeon CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? OpenAEONConfigFile.loadDict()
