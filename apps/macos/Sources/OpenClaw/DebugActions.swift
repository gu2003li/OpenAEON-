...
enum DebugActions {
    private static let verboseDefaultsKey = "openaeon.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "openaeon.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = OpenAEONPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "OpenAEON", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = OpenAEONPaths.configURL
        guard
