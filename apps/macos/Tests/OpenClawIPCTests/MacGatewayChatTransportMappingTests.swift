...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: OpenAEONProtocol.AnyCodable(["ok": OpenAEONProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: OpenAEONProtocol.AnyCodable(["ok": OpenAEONProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = OpenAEONProtocol.AnyCodable([
            "runId": OpenAEONProtocol.AnyCodable("run-1"),
            "sessionKey": OpenAEONProtocol.AnyCodable("main"),
            "state": OpenAEONProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: OpenAEONProtocol.AnyCodable(["a": OpenAEONProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
