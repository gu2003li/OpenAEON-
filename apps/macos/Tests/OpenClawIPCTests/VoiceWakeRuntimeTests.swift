...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["openaeon"]
        let text = "hey openaeon"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey openaeon do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["openaeon"], minPostTriggerGap: 0.3)
