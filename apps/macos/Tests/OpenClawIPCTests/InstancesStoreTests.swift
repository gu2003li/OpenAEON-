...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: OpenAEONProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: OpenAEONProtocol.AnyCodable] = [
            "presence": .init([OpenAEONProtocol.AnyCodable(entry)]),
        ]
        let payload = OpenAEONProtocol.AnyCodable(payloadMap)
