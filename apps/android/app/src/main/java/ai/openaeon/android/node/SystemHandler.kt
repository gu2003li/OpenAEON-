...
private const val NOTIFICATION_CHANNEL_BASE_ID = "openaeon.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "OpenAEON Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "OpenAEON Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "OpenAEON Active")
      }
