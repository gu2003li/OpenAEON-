...
import Foundation
import Testing
@testable import OpenAEONChatUI
...
private final class TestChatTransport: @unchecked Sendable, OpenAEONChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [OpenAEONChatHistoryPayload]
    private let sessionsResponses: [OpenAEONChatSessionsListResponse]

    private let stream: AsyncStream<OpenAEONChatTransportEvent>
    private let continuation: AsyncStream<OpenAEONChatTransportEvent>.Continuation

    init(
        historyResponses: [OpenAEONChatHistoryPayload],
        sessionsResponses: [OpenAEONChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<OpenAEONChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> OpenAEONChatHistoryPayload {
        ...
        return self.historyResponses.last ?? OpenAEONChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [OpenAEONChatAttachmentPayload]) async throws -> OpenAEONChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return OpenAEONChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> OpenAEONChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? OpenAEONChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: OpenAEONChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = OpenAEONChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = OpenAEONChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { OpenAEONChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = OpenAEONChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = OpenAEONChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = OpenAEONChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { OpenAEONChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
