...
@MainActor
struct OpenAEONChatComposer: View {
    @Bindable var viewModel: OpenAEONChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(OpenAEONChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(OpenAEONChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \OpenAEONPendingAttachment.id)
                { (att: OpenAEONPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            OpenAEONPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.isComposerCompacted {
                Rectangle()
                    .fill(OpenAEONChatTheme.divider)
                    .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(OpenAEONChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(OpenAEONChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(OpenAEONChatTheme.subtleCard)
        .clipShape(Capsule())
    }
