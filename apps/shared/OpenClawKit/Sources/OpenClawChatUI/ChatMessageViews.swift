...
struct ChatMessageBubble: View {
    let message: OpenAEONChatMessage
    let style: OpenAEONChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? OpenAEONChatTheme.userText : OpenAEONChatTheme.assistantText
    ...
    private var inlineAttachments: [OpenAEONChatMessageContent] {
    ...
    private var toolCalls: [OpenAEONChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [OpenAEONChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? OpenAEONChatTheme.userBubble
        }
        if self.style == .onboarding {
            return OpenAEONChatTheme.onboardingAssistantBubble
        }
        return OpenAEONChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return OpenAEONChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: OpenAEONChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? OpenAEONChatTheme.userText : OpenAEONChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: OpenAEONChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(OpenAEONChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(OpenAEONChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [OpenAEONChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: OpenAEONChatTheme.assistantText)
            }
        }
    }
}
