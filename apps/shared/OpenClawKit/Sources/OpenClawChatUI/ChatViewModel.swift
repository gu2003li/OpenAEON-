...
private let chatUILogger = Logger(subsystem: "ai.openaeon", category: "OpenAEONChatUI")

@MainActor
@Observable
public final class OpenAEONChatViewModel {
    public private(set) var messages: [OpenAEONChatMessage] = []
    ...
    public var attachments: [OpenAEONPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [OpenAEONChatPendingToolCall] = []
    public private(set) var sessions: [OpenAEONChatSessionEntry] = []
    private let transport: any OpenAEONChatTransport
    ...
    private var pendingToolCallsById: [String: OpenAEONChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any OpenAEONChatTransport) {
    ...
    public var sessionChoices: [OpenAEONChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [OpenAEONChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: OpenAEONPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [OpenAEONChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: OpenAEONChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: OpenAEONChatMessage) -> OpenAEONChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> OpenAEONChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return OpenAEONChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: OpenAEONChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [OpenAEONChatMessage],
        incoming: [OpenAEONChatMessage]) -> [OpenAEONChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [OpenAEONChatMessage]) -> [OpenAEONChatMessage] {
        var result: [OpenAEONChatMessage] = []
    ...
    private static func dedupeKey(for message: OpenAEONChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [OpenAEONChatMessageContent] = [
            OpenAEONChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> OpenAEONChatAttachmentPayload in
            OpenAEONChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> OpenAEONChatSessionEntry {
        OpenAEONChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: OpenAEONChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: OpenAEONChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: OpenAEONAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = OpenAEONChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> OpenAEONPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
