...
@MainActor
public struct OpenAEONChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: OpenAEONChatViewModel
    ...
    public init(
        viewModel: OpenAEONChatViewModel,
    ...
    private var visibleMessages: [OpenAEONChatMessage] {
        let base: [OpenAEONChatMessage]
    ...
    private func mergeToolResults(in messages: [OpenAEONChatMessage]) -> [OpenAEONChatMessage] {
        ...
        for message in messages {
            ...
            let merged = OpenAEONChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: OpenAEONChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func toolCallIds(in message: OpenAEONChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: OpenAEONChatMessage) -> String {
