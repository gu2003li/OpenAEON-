...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.openaeon.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to OpenAEON", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to OpenAEON gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to OpenAEON.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "OpenAEONShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "OpenAEON is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "OpenAEON Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("openaeon-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
