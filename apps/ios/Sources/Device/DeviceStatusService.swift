...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> OpenAEONDeviceStatusPayload {
        ...
        return OpenAEONDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> OpenAEONDeviceInfoPayload {
        ...
        return OpenAEONDeviceInfoPayload(
    ...
    private func batteryStatus() -> OpenAEONBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: OpenAEONBatteryState = switch device.batteryState {
        ...
        return OpenAEONBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> OpenAEONThermalStatusPayload {
        let state: OpenAEONThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return OpenAEONThermalStatusPayload(state: state)
    }

    private func storageStatus() -> OpenAEONStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return OpenAEONStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
