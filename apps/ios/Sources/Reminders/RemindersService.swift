...
final class RemindersService: RemindersServicing {
    func list(params: OpenAEONRemindersListParams) async throws -> OpenAEONRemindersListPayload {
        ...
        let payload: [OpenAEONReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return OpenAEONReminderPayload(
        ...
        return OpenAEONRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: OpenAEONRemindersAddParams) async throws -> OpenAEONRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = OpenAEONReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return OpenAEONRemindersAddPayload(reminder: payload)
    }
