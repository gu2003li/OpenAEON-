...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__OPENAEON_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<OpenAEONTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.openaeon", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = OpenAEONTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> OpenAEONTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return OpenAEONTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> OpenAEONTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> OpenAEONTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return OpenAEONTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> OpenAEONTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = OpenAEONTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: OpenAEONTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: OpenAEONAgentEventPayload.self
            ) else {
                continue
            }
