...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: OpenAEONWatchNotifyParams) -> OpenAEONWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [OpenAEONWatchAction]?,
        kind: String?,
        promptId: String?) -> [OpenAEONWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> OpenAEONWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return OpenAEONWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                OpenAEONWatchAction(id: "approve", label: "Approve"),
                OpenAEONWatchAction(id: "decline", label: "Decline", style: "destructive"),
                OpenAEONWatchAction(id: "open_phone", label: "Open iPhone"),
                OpenAEONWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            OpenAEONWatchAction(id: "done", label: "Done"),
            OpenAEONWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            OpenAEONWatchAction(id: "open_phone", label: "Open iPhone"),
            OpenAEONWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: OpenAEONWatchRisk?,
        priority: OpenAEONNotificationPriority?) -> OpenAEONWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: OpenAEONNotificationPriority?,
        risk: OpenAEONWatchRisk?) -> OpenAEONNotificationPriority?
    {
