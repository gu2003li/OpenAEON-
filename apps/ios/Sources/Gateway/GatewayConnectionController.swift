...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [OpenAEONCapability.canvas.rawValue, OpenAEONCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(OpenAEONCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(OpenAEONCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = OpenAEONLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(OpenAEONCapability.location.rawValue) }

        caps.append(OpenAEONCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(OpenAEONCapability.watch.rawValue)
        }
        caps.append(OpenAEONCapability.photos.rawValue)
        caps.append(OpenAEONCapability.contacts.rawValue)
        caps.append(OpenAEONCapability.calendar.rawValue)
        caps.append(OpenAEONCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(OpenAEONCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            OpenAEONScreenCommand.record.rawValue,
            OpenAEONSystemCommand.notify.rawValue,
            OpenAEONChatCommand.push.rawValue,
        ...
        if caps.contains(OpenAEONCapability.camera.rawValue) {
            commands.append(OpenAEONCameraCommand.list.rawValue)
            commands.append(OpenAEONCameraCommand.snap.rawValue)
            commands.append(OpenAEONCameraCommand.clip.rawValue)
        }
        if caps.contains(OpenAEONCapability.location.rawValue) {
            commands.append(OpenAEONLocationCommand.get.rawValue)
        }
        if caps.contains(OpenAEONCapability.device.rawValue) {
            commands.append(OpenAEONDeviceCommand.status.rawValue)
            commands.append(OpenAEONDeviceCommand.info.rawValue)
        }
        if caps.contains(OpenAEONCapability.watch.rawValue) {
            commands.append(OpenAEONWatchCommand.status.rawValue)
            commands.append(OpenAEONWatchCommand.notify.rawValue)
        }
        if caps.contains(OpenAEONCapability.photos.rawValue) {
            commands.append(OpenAEONPhotosCommand.latest.rawValue)
        }
        if caps.contains(OpenAEONCapability.contacts.rawValue) {
            commands.append(OpenAEONContactsCommand.search.rawValue)
            commands.append(OpenAEONContactsCommand.add.rawValue)
        }
        if caps.contains(OpenAEONCapability.calendar.rawValue) {
            commands.append(OpenAEONCalendarCommand.events.rawValue)
            commands.append(OpenAEONCalendarCommand.add.rawValue)
        }
        if caps.contains(OpenAEONCapability.reminders.rawValue) {
            commands.append(OpenAEONRemindersCommand.list.rawValue)
            commands.append(OpenAEONRemindersCommand.add.rawValue)
        }
        if caps.contains(OpenAEONCapability.motion.rawValue) {
            commands.append(OpenAEONMotionCommand.activity.rawValue)
            commands.append(OpenAEONMotionCommand.pedometer.rawValue)
        }
