...
final class ContactsService: ContactsServicing {
    ...
    func search(params: OpenAEONContactsSearchParams) async throws -> OpenAEONContactsSearchPayload {
        ...
        return OpenAEONContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: OpenAEONContactsAddParams) async throws -> OpenAEONContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return OpenAEONContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return OpenAEONContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> OpenAEONContactPayload {
        OpenAEONContactPayload(
