...
final class MotionService: MotionServicing {
    func activities(params: OpenAEONMotionActivityParams) async throws -> OpenAEONMotionActivityPayload {
        ...
        let mapped: [OpenAEONMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return OpenAEONMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: OpenAEONPedometerParams) async throws -> OpenAEONPedometerPayload {
        ...
        let payload: OpenAEONPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = OpenAEONPedometerPayload(
