...
struct IOSGatewayChatTransport: OpenAEONChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.openaeon", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> OpenAEONChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(OpenAEONChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> OpenAEONChatHistoryPayload {
        ...
        return try JSONDecoder().decode(OpenAEONChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [OpenAEONChatAttachmentPayload]) async throws -> OpenAEONChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [OpenAEONChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(OpenAEONChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(OpenAEONGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenAEONGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenAEONChatEventPayload.self)
                        {
