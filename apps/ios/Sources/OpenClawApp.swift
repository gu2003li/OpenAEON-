...
@MainActor
final class OpenAEONAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.openaeon.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.openaeon.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.openaeon.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "openaeon.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "openaeon.watch.promptId"
    static let sessionKeyKey = "openaeon.watch.sessionKey"
    static let actionPrimaryIDKey = "openaeon.watch.action.primary.id"
    static let actionPrimaryLabelKey = "openaeon.watch.action.primary.label"
    static let actionSecondaryIDKey = "openaeon.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "openaeon.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "openaeon.watch.action.primary"
    static let actionSecondaryIdentifier = "openaeon.watch.action.secondary"
    static let actionIdentifierPrefix = "openaeon.watch.action."
    static let actionIDKeyPrefix = "openaeon.watch.action.id."
    static let actionLabelKeyPrefix = "openaeon.watch.action.label."
    static let categoryPrefix = "openaeon.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: OpenAEONWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> OpenAEONWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return OpenAEONWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "OpenAEON" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [OpenAEONWatchAction]) -> [UNNotificationAction] {
...
@main
struct OpenAEONApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(OpenAEONAppDelegate.self) private var appDelegate
...
extension OpenAEONApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("OpenAEON: installing uncaught exception handler")
