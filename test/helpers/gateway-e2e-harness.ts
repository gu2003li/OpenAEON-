...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `openaeon-e2e-${name}-`));
  const configDir = path.join(homeDir, ".openaeon");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OPENAEON_SKIP_PROVIDERS: "1",
          OPENAEON_SKIP_GMAIL_WATCHER: "1",
          OPENAEON_SKIP_CRON: "1",
          OPENAEON_SKIP_BROWSER_CONTROL_SERVER: "1",
          OPENAEON_SKIP_CANVAS_HOST: "1",
          OPENAEON_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
