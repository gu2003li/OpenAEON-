...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    openaeonHome: process.env.OPENAEON_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("OPENAEON_HOME", snapshot.openaeonHome);
  restoreKey("OPENAEON_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked OPENAEON_HOME.
  delete process.env.OPENAEON_HOME;
  process.env.OPENAEON_STATE_DIR = path.join(base, ".openaeon");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "openaeon-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".openaeon", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
