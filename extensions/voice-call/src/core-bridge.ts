...
function resolveOpenAEONRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.OPENAEON_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set OPENAEON_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveOpenAEONRoot(), "dist", "extensionAPI.js");
