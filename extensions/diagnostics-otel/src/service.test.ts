...
vi.mock("openaeon/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("openaeon/plugin-sdk")>("openaeon/plugin-sdk");
  return {
    ...actual,
...
import type { OpenAEONPluginServiceContext } from "openaeon/plugin-sdk";
import { emitDiagnosticEvent } from "openaeon/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/openaeon-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): OpenAEONPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("openaeon.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("openaeon.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("openaeon.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("openaeon.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("openaeon.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("openaeon.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("openaeon.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("openaeon.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("openaeon.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("openaeon.webhook.processed");
    expect(spanNames).toContain("openaeon.message.processed");
    expect(spanNames).toContain("openaeon.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["openaeon.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("openaeon.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "openaeon.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["openaeon.reason"]).toBe("string");
    expect(String(attrs?.["openaeon.reason"])).not.toContain(
