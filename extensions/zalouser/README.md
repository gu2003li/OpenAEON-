...
OpenAEON extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
openaeon channels login --channel zalouser
...
```bash
openaeon message send --channel zalouser --target <threadId> --message "Hello from OpenAEON!"
```
...
```bash
openaeon channels login --channel zalouser              # Login via QR
openaeon channels login --channel zalouser --account work
openaeon channels status --probe
openaeon channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
openaeon directory self --channel zalouser
openaeon directory peers list --channel zalouser --query "name"
openaeon directory groups list --channel zalouser --query "work"
openaeon directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
openaeon message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
openaeon message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `openaeon channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
openaeon channels login --channel zalouser --account work
openaeon message send --channel zalouser --account work --target <id> --message "Hello"
