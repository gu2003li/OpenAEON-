...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["openaeonPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["openaeon", "pollId"]) ??
    readNestedString(value, ["openaeon", "poll", "id"]) ??
    readNestedString(value, ["data", "openaeonPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "openaeon", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          openaeonPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "openaeon poll vote",
          displayText: "Vote recorded",
          value: { openaeonPollId: pollId, pollId },
        },
      },
    ],
  };
