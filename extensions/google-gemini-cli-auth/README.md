# Google Gemini CLI Auth (OpenAEON plugin)
...
```bash
openaeon plugins enable google-gemini-cli-auth
```
...
```bash
openaeon models auth login --provider google-gemini-cli --set-default
```
...
- `OPENAEON_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `OPENAEON_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
