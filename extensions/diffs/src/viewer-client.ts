...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  return [...document.querySelectorAll<HTMLElement>(".oc-diff-card")].flatMap((card) => {
    const host = card.querySelector<HTMLElement>("[data-openaeon-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-openaeon-diff-payload]");
    if (!host || !payloadNode) {
      return [];
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.openaeonDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.openaeonDiffsError = "true";
