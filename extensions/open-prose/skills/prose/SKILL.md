...
metadata: { "openaeon": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == OpenAEON `sessions_spawn`
- **File I/O** == OpenAEON `read`/`write`
- **Remote fetch** == OpenAEON `web_fetch` (or `exec` with curl when POST is required)
