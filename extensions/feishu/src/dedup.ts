...
import { createDedupeCache, createPersistentDedupe } from "openaeon/plugin-sdk";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.OPENAEON_STATE_DIR?.trim() || env.AEONPROPHET_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["openaeon-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".openaeon");
}
