import type { AnyAgentTool, OpenAEONPluginApi } from "openaeon/plugin-sdk";
...
export function createToolFactoryHarness(cfg: OpenAEONPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<OpenAEONPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
