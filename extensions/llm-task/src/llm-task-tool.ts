...
import Ajv from "ajv";
import { resolvePreferredOpenAEONTmpDir } from "openaeon/plugin-sdk";
// NOTE: This extension is intended to be bundled with OpenAEON.
// When running from source (tests/dev), OpenAEON internals live under src/.
// When running from a built install, internals live under dist/ (no src/ tree).
// So we resolve internal imports dynamically with src-first, dist-fallback.
import type { OpenAEONPluginApi } from "../../../src/plugins/types.js";
...
export function createLlmTaskTool(api: OpenAEONPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via openaeon.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredOpenAEONTmpDir(), "openaeon-llm-task-"),
        );
