...
`openaeon.invoke --each`) without adding new OpenAEON code per workflow.
...
This extension depends on OpenAEON internal modules (the embedded agent runner).
It is intended to ship as a **bundled** OpenAEON extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.openaeon/extensions` as a standalone plugin directory.
