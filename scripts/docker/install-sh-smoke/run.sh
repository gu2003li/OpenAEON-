...
INSTALL_URL="${OPENAEON_INSTALL_URL:-https://openaeon.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${OPENAEON_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${OPENAEON_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="openaeon"
PACKAGE_NAME="${OPENAEON_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (OPENAEON_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${OPENAEON_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${OPENAEON_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
