...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/openaeon/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "OpenAEON Re-Auth" -c "After getting key, run: ssh $SERVER '~/openaeon/scripts/mobile-reauth.sh'" --id openaeon-auth
        ;;
    *)
