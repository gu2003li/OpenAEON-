 ...
 * `openaeon update`. Use it to verify the completion installation flow
...
\`openaeon update\`, \`openaeon doctor\`, and \`openaeon onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# OpenAEON Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`openaeon completion --install\` later to enable.`));
    return;
  }
