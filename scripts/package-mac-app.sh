...
# Build and bundle OpenAEON into a minimal .app we can open.
# Outputs to dist/OpenAEON.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/OpenAEON.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="OpenAEON"
BUNDLE_ID="${BUNDLE_ID:-ai.openaeon.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/openaeon/openaeon/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/OpenAEON/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OpenAEONBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OpenAEONGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/OpenAEON"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/OpenAEON"
fi
chmod +x "$APP_ROOT/Contents/MacOS/OpenAEON"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/OpenAEON" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/OpenAEON/Resources/OpenAEON.icns" "$APP_ROOT/Contents/Resources/OpenAEON.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/OpenAEON/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OpenAEONKit resources"
OPENAEONKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OpenAEONKit_OpenAEONKit.bundle"
if [ -d "$OPENAEONKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OpenAEONKit_OpenAEONKit.bundle"
  cp -R "$OPENAEONKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OpenAEONKit_OpenAEONKit.bundle"
else
  echo "WARN: OpenAEONKit resource bundle not found at $OPENAEONKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running OpenAEON"
killall -q OpenAEON 2>/dev/null || true
