...
#
# Suggested cron: */30 * * * * /home/admin/openaeon/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send OpenAEON notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., openaeon-alerts)
...
STATE_FILE="$HOME/.openaeon/auth-monitor-state"
...
send_notification() {
    ...
    # Send via OpenAEON if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use openaeon
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OpenAEON to $NOTIFY_PHONE..."
            openaeon send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OpenAEON is down. Run: ssh l36 '~/openaeon/scripts/mobile-reauth.sh'" "urgent"
    exit 1
