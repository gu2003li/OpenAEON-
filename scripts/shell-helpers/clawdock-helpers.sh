#!/usr/bin/env bash
# ClawDock - Docker helpers for OpenAEON
# Inspired by Simon Willison's "Running OpenAEON in Docker"
# https://til.simonwillison.net/llms/openaeon-docker
#
# Installation:
...
# Common paths to check for OpenAEON
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/openaeon"
  "${HOME}/workspace/openaeon"
  "${HOME}/projects/openaeon"
  "${HOME}/dev/openaeon"
  "${HOME}/code/openaeon"
  "${HOME}/src/openaeon"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found OpenAEON at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/openaeon"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ OpenAEON not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/openaeon/openaeon.git ~/openaeon"
    echo "  cd ~/openaeon && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^OPENAEON_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d openaeon-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart openaeon-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f openaeon-gateway
}
...
clawdock-workspace() {
  cd ~/.openaeon/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec openaeon-gateway \
    bash -c 'echo "alias openaeon=\"./openaeon.mjs\"" > /tmp/.bashrc_openaeon && bash --rcfile /tmp/.bashrc_openaeon'
}
...
clawdock-cli() {
  _clawdock_compose run --rm openaeon-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build openaeon-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "OPENAEON_GATEWAY_TOKEN=$token" openaeon-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" openaeon-gateway \
    bash -c './openaeon.mjs config set gateway.remote.token "$TOKEN" && ./openaeon.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec openaeon-gateway \
    bash -c "./openaeon.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart openaeon-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm openaeon-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec openaeon-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'openaeon config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for OpenAEON${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (openaeon alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to openaeon project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.openaeon)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'openaeon channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'openaeon status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.openaeon.ai${_CLR_RESET}"
  echo ""
}
