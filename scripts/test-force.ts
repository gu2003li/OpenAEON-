...
function runTests() {
  const isolatedLock =
    process.env.OPENAEON_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `openaeon-gateway.lock.test.${Date.now()}`);
  const result = spawnSync("pnpm", ["vitest", "run"], {
    stdio: "inherit",
    env: {
      ...process.env,
      OPENAEON_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
...
function main() {
  const port = Number.parseInt(process.env.OPENAEON_GATEWAY_PORT ?? `${DEFAULT_PORT}`, 10);
