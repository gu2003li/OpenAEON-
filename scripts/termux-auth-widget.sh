...
# OpenAEON Auth Widget for Termux
...
# Check auth status
termux-toast "Checking OpenAEON auth..."

STATUS=$(ssh "$SERVER" '$HOME/openaeon/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/openaeon/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|OPENAEON_EXPIRING|AEONPROPHET_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/openaeon/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|OPENAEON_EXPIRED|AEONPROPHET_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: OpenAEON won't work until re-auth"
                ;;
        esac
        ;;
