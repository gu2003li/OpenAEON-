...
show_usage() {
    cat << EOF
clawlog - OpenAEON Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View OpenAEON logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    OpenAEON logs flow through the macOS unified log (subsystem: ai.openaeon).
