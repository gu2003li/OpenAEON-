...
IMAGE_NAME="openaeon-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="openaeon-net-e2e-$$"
GW_NAME="openaeon-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "OPENAEON_GATEWAY_TOKEN=$TOKEN" \
  -e "OPENAEON_SKIP_CHANNELS=1" \
  -e "OPENAEON_SKIP_GMAIL_WATCHER=1" \
  -e "OPENAEON_SKIP_CRON=1" \
  -e "OPENAEON_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
