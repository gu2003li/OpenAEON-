#!/bin/bash
# Setup OpenAEON Auth Management System
...
echo "=== OpenAEON Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'openaeon-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OpenAEON message: Send warning via OpenAEON itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/openaeon-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/openaeon-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/openaeon-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now openaeon-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status openaeon-auth-monitor.timer"
echo ""
echo "Quick commands:"
