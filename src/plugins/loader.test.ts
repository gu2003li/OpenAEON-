...
let tempDirIndex = 0;
const prevBundledDir = process.env.OPENAEON_BUNDLED_PLUGINS_DIR;
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.OPENAEON_BUNDLED_PLUGINS_DIR = bundledDir;
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.OPENAEON_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.OPENAEON_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadOpenClawPlugins", () => {
  ...
  it("loads plugins from config paths", () => {
    process.env.OPENAEON_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    withEnv({ OPENAEON_STATE_DIR: stateDir, AEONPROPHET_STATE_DIR: undefined }, () => {
