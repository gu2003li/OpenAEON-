...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: OPENAEONConfig, accountId: string | null | undefined) => unknown;
...
describe("security audit", () => {
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync(
      { OPENAEON_STATE_DIR: channelSecurityStateDir },
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.OPENAEON_GATEWAY_TOKEN;
    const prevPassword = process.env.OPENAEON_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.OPENAEON_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.OPENAEON_GATEWAY_PASSWORD;
      } else {
        process.env.OPENAEON_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): OPENAEONConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: OPENAEONConfig = { session: { dmScope: "main" } };
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: OPENAEONConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.OPENAEON_GATEWAY_TOKEN;
    process.env.OPENAEON_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: OPENAEONConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<OPENAEONConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: OPENAEONConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    try {
      const cfg: OPENAEONConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.OPENAEON_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.OPENAEON_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
