...
This directory contains hooks that ship with OPENAEON. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.openaeon/openaeon.json`:
...
2. Restart gateway: `pkill -9 -f 'openaeon.*gateway' && pnpm openaeon gateway`
3. Enable the hook: `openaeon hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.openaeon.ai/automation/hooks
