...
homepage: https://docs.openaeon.ai/automation/hooks#command-logger
metadata:
  {
    "openaeon":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.openaeon/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.openaeon/logs/commands.log ~/.openaeon/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/openaeon`:
   ```
   /home/username/.openaeon/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
