...
export function resolveConfigDir(
  ...
  const override = env.OPENAEON_STATE_DIR?.trim() || env.AEONPROPHET_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override);
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".openaeon");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.OPENAEON_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$OPENAEON_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via OPENAEON_STATE_DIR.
