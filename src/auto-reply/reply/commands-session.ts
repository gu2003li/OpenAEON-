...
import { scheduleGatewaySigusr1Restart, triggerOPENAEONRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting OPENAEON in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerOPENAEONRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting OPENAEON via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
