...
export function resolveConfiguredAcpBackendId(cfg: OPENAEONConfig): string {
  return cfg.acp?.backend?.trim() || "acpx";
}

export function resolveAcpInstallCommandHint(cfg: OPENAEONConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(localPath)) {
      return `openaeon plugins install ${localPath}`;
    }
    return "openaeon plugins install @openaeon/acpx";
  }
