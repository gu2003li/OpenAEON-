...
export function isCommandEnabled(cfg: OPENAEONConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OPENAEONConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OPENAEONConfig),
...
export function getCommandDetection(_cfg?: OPENAEONConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OPENAEONConfig) {
