...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.OPENAEON_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  const stateDir = env.OPENAEON_STATE_DIR;
  const configPath = env.OPENAEON_CONFIG_PATH;
  ...
  return {
    ...
    OPENAEON_PROFILE: profile,
    OPENAEON_STATE_DIR: stateDir,
    OPENAEON_CONFIG_PATH: configPath,
    OPENAEON_GATEWAY_PORT: String(port),
    OPENAEON_GATEWAY_TOKEN: token,
    OPENAEON_LAUNCHD_LABEL: resolvedLaunchdLabel,
    OPENAEON_SYSTEMD_UNIT: systemdUnit,
    OPENAEON_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    OPENAEON_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const platform = params.platform ?? process.platform;
  const gatewayToken =
    env.OPENAEON_GATEWAY_TOKEN?.trim() || env.AEONPROPHET_GATEWAY_TOKEN?.trim() || undefined;
  const stateDir = env.OPENAEON_STATE_DIR;
  const configPath = env.OPENAEON_CONFIG_PATH;
  const tmpDir = env.TMPDIR?.trim() || os.tmpdir();
  const proxyEnv = readServiceProxyEnvironment(env);
  ...
  return {
    ...
    OPENAEON_STATE_DIR: stateDir,
    OPENAEON_CONFIG_PATH: configPath,
    OPENAEON_GATEWAY_TOKEN: gatewayToken,
    OPENAEON_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    OPENAEON_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    OPENAEON_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    OPENAEON_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    OPENAEON_LOG_PREFIX: "node",
    OPENAEON_SERVICE_MARKER: NODE_SERVICE_MARKER,
    OPENAEON_SERVICE_KIND: NODE_SERVICE_KIND,
    OPENAEON_SERVICE_VERSION: VERSION,
  };
}
