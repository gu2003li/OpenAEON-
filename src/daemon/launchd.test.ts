...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.openaeon.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", OPENAEON_PROFILE: "default" },
    });
...
describe("launchd bootstrap repair", () => {
  it("bootstraps and kickstarts the resolved label", async () => {
    const env: Record<string, string | undefined> = {
      HOME: "/Users/test",
      OPENAEON_PROFILE: "default",
    };
    const repair = await repairLaunchAgentBootstrap({ env });
    expect(repair.ok).toBe(true);

    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const label = "ai.openaeon.gateway";
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when OPENAEON_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.openaeon.gateway.plist",
    },
    {
      name: "uses profile-specific label when OPENAEON_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", OPENAEON_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.openaeon.jbphoenix.plist",
    },
    {
      name: "prefers OPENAEON_LAUNCHD_LABEL over OPENAEON_PROFILE",
      env: {
        HOME: "/Users/test",
        OPENAEON_PROFILE: "jbphoenix",
        OPENAEON_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from OPENAEON_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        OPENAEON_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty OPENAEON_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        OPENAEON_PROFILE: "myprofile",
        OPENAEON_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.openaeon.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
