...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.OPENAEON_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service OPENAEON_GATEWAY_TOKEN does not match gateway.auth.token in openaeon.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `openaeon gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
