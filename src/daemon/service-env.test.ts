...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPENAEON_GATEWAY_PORT).toBe("18789");
    expect(env.OPENAEON_GATEWAY_TOKEN).toBe("secret");
    expect(env.OPENAEON_SERVICE_MARKER).toBe("openclaw");
    expect(env.OPENAEON_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPENAEON_SERVICE_VERSION).toBe("string");
    expect(env.OPENAEON_SYSTEMD_UNIT).toBe("openclaw-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OPENAEON_LAUNCHD_LABEL).toBe("ai.openaeon.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OPENAEON_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OPENAEON_SYSTEMD_UNIT).toBe("openclaw-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.OPENAEON_LAUNCHD_LABEL).toBe("ai.openaeon.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through OPENAEON_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", OPENAEON_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.OPENAEON_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy AEONPROPHET_GATEWAY_TOKEN to OPENAEON_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", AEONPROPHET_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.OPENAEON_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers OPENAEON_GATEWAY_TOKEN over legacy AEONPROPHET_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        OPENAEON_GATEWAY_TOKEN: "openclaw-token",
        AEONPROPHET_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.OPENAEON_GATEWAY_TOKEN).toBe("openclaw-token");
  });

  it("omits OPENAEON_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        OPENAEON_GATEWAY_TOKEN: "   ",
        AEONPROPHET_GATEWAY_TOKEN: " ",
      },
    });
    expect(env.OPENAEON_GATEWAY_TOKEN).toBeUndefined();
  });
...
describe("resolveGatewayStateDir", () => {
  ...
  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", OPENAEON_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", OPENAEON_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openclaw"));
  });

  it("uses OPENAEON_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", OPENAEON_STATE_DIR: "/var/lib/openclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/openclaw"));
  });

  it("expands ~ in OPENAEON_STATE_DIR", () => {
    const env = { HOME: "/Users/test", OPENAEON_STATE_DIR: "~/openclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/openclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { OPENAEON_STATE_DIR: "C:\\State\\openclaw" };
