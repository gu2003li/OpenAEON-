...
function listAgents(cfg: OPENAEONConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: OPENAEONConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: OPENAEONConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<OPENAEONConfig, EvaluatedBindingsCache>();
