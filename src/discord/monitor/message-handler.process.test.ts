...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn(() => "/tmp/openaeon-discord-process-test-sessions.json"),
}));
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/openaeon-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/openaeon-discord-process-test-sessions.json" },
      },
    });
