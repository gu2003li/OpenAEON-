...
function resolveVoiceTtsConfig(params: { cfg: OPENAEONConfig; override?: TtsConfig }): {
  cfg: OPENAEONConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredOPENAEONTmpDir(), "discord-voice-"));
