...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.openaeon.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + OPENAEON hooks")
    ...
    .option("--hook-url <url>", "OPENAEON hook URL")
    .option("--hook-token <token>", "OPENAEON hook token")
