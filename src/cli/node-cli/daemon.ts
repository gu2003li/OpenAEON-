...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("openaeon node install"), formatCliCommand("openaeon node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("openaeon node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    OPENAEON_LOG_PREFIX: baseEnv.OPENAEON_LOG_PREFIX ?? "node",
