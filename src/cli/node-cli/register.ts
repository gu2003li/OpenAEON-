    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "openaeon node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["openaeon node status", "Check node host service status."],
          ["openaeon node install", "Install the node host service."],
          ["openaeon node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.openaeon.ai/cli/node")}\n`,
    );
