...
${formatHelpExamples([
  ['openaeon agent --to +15555550123 --message "status update"', "Start a new session."],
  ['openaeon agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'openaeon agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'openaeon agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['openaeon agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'openaeon agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.openaeon.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.openaeon.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['openaeon agents set-identity --agent main --name "OPENAEON" --emoji "🦞"', "Set name + emoji."],
  ["openaeon agents set-identity --agent main --avatar avatars/openaeon.png", "Set avatar path."],
  [
    "openaeon agents set-identity --workspace ~/.openaeon/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "openaeon agents set-identity --identity-file ~/.openaeon/workspace/IDENTITY.md --agent main",
