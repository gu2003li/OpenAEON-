...
beforeEach(() => {
  ...
  originalHideBanner = process.env.OPENAEON_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.OPENAEON_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.OPENAEON_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  it("hides banner when OPENAEON_HIDE_BANNER is truthy", async () => {
    process.env.OPENAEON_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
