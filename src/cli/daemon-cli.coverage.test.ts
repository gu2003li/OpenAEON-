...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.OPENAEON_STATE_DIR = "/tmp/openclaw-cli-state";
    process.env.OPENAEON_CONFIG_PATH = "/tmp/openclaw-cli-state/openclaw.json";
    delete process.env.OPENAEON_GATEWAY_PORT;
    delete process.env.OPENAEON_PROFILE;
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        OPENAEON_STATE_DIR: "/tmp/openclaw-daemon-state",
        OPENAEON_CONFIG_PATH: "/tmp/openclaw-daemon-state/openclaw.json",
        OPENAEON_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.openaeon.gateway.plist",
    });
