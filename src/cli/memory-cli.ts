    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openaeon memory status", "Show index and provider status."],
          ["openaeon memory index --force", "Force a full reindex."],
          ['openaeon memory search --query "deployment notes"', "Search indexed memory entries."],
          ["openaeon memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.openaeon.ai/cli/memory")}\n`,
    );
