...
const SANDBOX_EXAMPLES = {
  main: [
    ["openaeon sandbox list", "List all sandbox containers."],
    ["openaeon sandbox list --browser", "List only browser containers."],
    ["openaeon sandbox recreate --all", "Recreate all containers."],
    ["openaeon sandbox recreate --session main", "Recreate a specific session."],
    ["openaeon sandbox recreate --agent mybot", "Recreate agent containers."],
    ["openaeon sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["openaeon sandbox list", "List all sandbox containers."],
    ["openaeon sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["openaeon sandbox recreate --all", "Recreate all containers."],
    ["openaeon sandbox recreate --session main", "Recreate a specific session."],
    ["openaeon sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["openaeon sandbox recreate --browser --all", "Recreate only browser containers."],
    ["openaeon sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["openaeon sandbox explain", "Show effective sandbox config."],
    ["openaeon sandbox explain --session agent:main:main", "Explain a specific session."],
    ["openaeon sandbox explain --agent work", "Explain an agent sandbox."],
    ["openaeon sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.openaeon.ai/cli/sandbox")}\n`,
    )
    .action(() => {
