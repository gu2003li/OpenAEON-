    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openaeon nodes status", "List known nodes with live status."],
          ["openaeon nodes pairing pending", "Show pending node pairing requests."],
          ['openaeon nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["openaeon nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.openaeon.ai/cli/nodes")}\n`,
    );
