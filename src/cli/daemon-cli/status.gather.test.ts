...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    OPENAEON_STATE_DIR: "/tmp/openclaw-daemon",
    OPENAEON_CONFIG_PATH: "/tmp/openclaw-daemon/openclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.OPENAEON_STATE_DIR ?? "/tmp/openclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.OPENAEON_CONFIG_PATH ?? `${stateDir}/openclaw.json`;
});
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.OPENAEON_STATE_DIR = "/tmp/openclaw-cli";
    process.env.OPENAEON_CONFIG_PATH = "/tmp/openclaw-cli/openclaw.json";
