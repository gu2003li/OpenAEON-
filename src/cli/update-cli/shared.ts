...
const OPENAEON_REPO_URL = "https://github.com/openaeon/openaeon.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "openaeon";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("openaeon@")) {
    return trimmed.slice("openaeon@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.OPENAEON_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveOPENAEONPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", OPENAEON_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OPENAEON_GIT_DIR points at a non-git directory: ${params.dir}. Set OPENAEON_GIT_DIR to an empty folder or an openaeon checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`OPENAEON_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "openaeon.mjs");
