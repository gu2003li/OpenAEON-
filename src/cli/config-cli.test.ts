...
import type { ConfigFileSnapshot, OPENAEONConfig } from "../config/types.js";
...
const mockWriteConfigFile = vi.fn<
  (cfg: OPENAEONConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: OPENAEONConfig, options?: { unsetPaths?: string[][] }) =>
...
function setSnapshot(resolved: OPENAEONConfig, config: OPENAEONConfig) {
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: OPENAEONConfig = {
        ...resolved,
        agents: {
          ...resolved.agents,
          defaults: {
