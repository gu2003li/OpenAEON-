...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update OPENAEON and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["openaeon update", "Update a source checkout (git)"],
        ["openaeon update --channel beta", "Switch to beta channel (git + npm)"],
        ["openaeon update --channel dev", "Switch to dev channel (git + npm)"],
        ["openaeon update --tag beta", "One-off update to a dist-tag or version"],
        ["openaeon update --dry-run", "Preview actions without changing anything"],
        ["openaeon update --no-restart", "Update without restarting the service"],
        ["openaeon update --json", "Output result as JSON"],
        ["openaeon update --yes", "Non-interactive (accept downgrade prompts)"],
        ["openaeon update wizard", "Interactive update wizard"],
        ["openaeon --update", "Shorthand for openaeon update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run openaeon update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.openaeon.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openaeon update status", "Show channel + version status."],
          ["openaeon update status --json", "JSON output."],
          ["openaeon update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.openaeon.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
