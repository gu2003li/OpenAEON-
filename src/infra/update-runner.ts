...
const DEFAULT_PACKAGE_NAME = "openaeon";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openaeon-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "openaeon.mjs");
    ...
    const doctorStep = await runStep(
      step("openaeon doctor", doctorArgv, gitRoot, { OPENAEON_UPDATE_IN_PROGRESS: "1" }),
    );
