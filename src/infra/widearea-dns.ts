...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OPENAEON_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*openaeon-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "openaeon";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "openaeon");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "openaeon-gw");
  ...
  records.push(`_openaeon-gw._tcp IN PTR ${instanceLabel}._openaeon-gw._tcp`);
  records.push(`${instanceLabel}._openaeon-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._openaeon-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; openaeon-content-hash: ${contentHash}\n${contentBody}`;
}
