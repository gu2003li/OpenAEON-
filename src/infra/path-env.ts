...
function candidateBinDirs(opts: EnsureOPENAEONPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `openaeon` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "openaeon");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.OPENAEON_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "openaeon"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `openaeon` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOPENAEONCliOnPath(opts: EnsureOPENAEONPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPENAEON_PATH_BOOTSTRAPPED)) {
    return;
  }
