...
function queuePendingWakeReason(params?: {
  ...
  if (previous.reasons.indexOf(normalizedReason) === -1) {
    previous.reasons.push(normalizedReason);
  ...
  timer = setTimeout(async () => {
    ...
    try {
      for (const pendingWake of pendingBatch) {
        const wakeOpts = {
          reasons: pendingWake.reasons,
          reason: pendingWake.reasons[0] ?? undefined,
        ...
        if (res.status === "skipped" && res.reason === "requests-in-flight") {
          ...
          queuePendingWakeReason({
            reason: pendingWake.reasons[0] ?? "retry",
