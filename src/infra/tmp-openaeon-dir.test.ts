...
import { POSIX_OPENAEON_TMP_DIR, resolvePreferredOPENAEONTmpDir } from "./tmp-openaeon-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredOPENAEONTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `openaeon-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const wrappedLstatSync = vi.fn((target: string) => {
    if (target === POSIX_OPENAEON_TMP_DIR) {
  ...
  const resolved = resolvePreferredOPENAEONTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredOPENAEONTmpDir", () => {
  it("prefers /tmp/openaeon when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_OPENAEON_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/openaeon when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_OPENAEON_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/openaeon when /tmp/openaeon is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/openaeon when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/openaeon is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/openaeon is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/openaeon is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback OPENAEON temp dir/);
  });
