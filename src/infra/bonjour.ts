...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OPENAEON_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "OPENAEON";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(OPENAEON\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.OPENAEON_MDNS_HOSTNAME?.trim() ||
    process.env.AEONPROPHET_MDNS_HOSTNAME?.trim() ||
    "openaeon";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
