...
vi.mock("./openaeon-root.js", () => ({
  resolveOPENAEONPackageRoot: vi.fn(async () => null),
  resolveOPENAEONPackageRootSync: vi.fn(() => null),
}));
...
let openaeonRoot: typeof import("./openaeon-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    openaeonRoot = await import("./openaeon-root.js");
  });
  ...
  it("uses resolveOPENAEONPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/openaeon");
    (
      openaeonRoot.resolveOPENAEONPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/openaeon"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "openaeon" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "openaeon.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-openaeon");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/openaeon-bundle");
    (
      openaeonRoot.resolveOPENAEONPackageRootSync as unknown as ReturnType<typeof vi.fn>
