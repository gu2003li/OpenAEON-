...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.OPENAEON_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.OPENAEON_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set OPENAEON_APNS_TEAM_ID and OPENAEON_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.OPENAEON_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.OPENAEON_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.OPENAEON_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set OPENAEON_APNS_PRIVATE_KEY_P8 or OPENAEON_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading OPENAEON_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createOPENAEONPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    openaeon: createOPENAEONPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
