...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: OPENAEONConfig | null = null;
  ...
  get(key: string, cfg: OPENAEONConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: OPENAEONConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: OPENAEONConfig): void {
