...
import { resolvePreferredOPENAEONTmpDir } from "../tmp-openaeon-dir.js";
...
const telegramChunkConfig: OPENAEONConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: OPENAEONConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes OPENAEON tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredOPENAEONTmpDir()]),
      }),
    );
  });

  it("includes OPENAEON tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes OPENAEON tmp root in whatsapp mediaLocalRoots", async () => {
  ...
  it("includes OPENAEON tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: OPENAEONConfig = { channels: { signal: { mediaMaxMb: 2 } } };
