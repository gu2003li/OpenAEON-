...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred OPENAEON tmp root", async () => {
    const tmpRoot = resolvePreferredOPENAEONTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideOPENAEON = path.join(os.tmpdir(), "outside-openaeon", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
