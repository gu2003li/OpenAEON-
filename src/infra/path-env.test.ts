...
let ensureOPENAEONCliOnPath: typeof import("./path-env.js").ensureOPENAEONCliOnPath;

describe("ensureOPENAEONCliOnPath", () => {
  const envKeys = [
    "PATH",
    "OPENAEON_PATH_BOOTSTRAPPED",
    "OPENAEON_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureOPENAEONCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling openaeon exists", () => {
    const tmp = abs("/tmp/openaeon-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "openaeon");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.OPENAEON_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.OPENAEON_PATH_BOOTSTRAPPED = "1";
    ensureOPENAEONCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/openaeon-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "openaeon");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/openaeon-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "openaeon");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "openaeon");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/openaeon-path/case-linuxbrew");
