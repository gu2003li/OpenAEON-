...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__openaeon_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "openaeon-root");
...
describe("resolveOPENAEONPackageRoot", () => {
  let resolveOPENAEONPackageRoot: typeof import("./openaeon-root.js").resolveOPENAEONPackageRoot;
  let resolveOPENAEONPackageRootSync: typeof import("./openaeon-root.js").resolveOPENAEONPackageRootSync;

  beforeAll(async () => {
    ({ resolveOPENAEONPackageRoot, resolveOPENAEONPackageRootSync } =
      await import("./openaeon-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "openaeon");
    const pkgRoot = path.join(project, "node_modules", "openaeon");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "openaeon" }));

    expect(resolveOPENAEONPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "openaeon");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "openaeon.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "openaeon" }));

    expect(resolveOPENAEONPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveOPENAEONPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-openaeon package roots", async () => {
    const pkgRoot = fx("not-openaeon");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-openaeon" }));

    expect(resolveOPENAEONPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveOPENAEONPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveOPENAEONPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
