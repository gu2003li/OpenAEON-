...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __openaeonDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__openaeonDiagnosticEventsState) {
    globalStore.__openaeonDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__openaeonDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: OPENAEONConfig): boolean {
