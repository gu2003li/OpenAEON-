...
vi.mock("./openaeon-root.js", () => ({
  resolveOPENAEONPackageRoot: vi.fn(),
}));
...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openaeon-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["OPENAEON_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.OPENAEON_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveOPENAEONPackageRoot } = await import("./openaeon-root.js"));
    ...
    vi.mocked(resolveOPENAEONPackageRoot).mockClear();
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolveOPENAEONPackageRoot).mockResolvedValue("/opt/openaeon");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/openaeon/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
