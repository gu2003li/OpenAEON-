...
export function triggerOPENAEONRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OPENAEON_SYSTEMD_UNIT,
        process.env.OPENAEON_PROFILE,
      );
  ...
  const label =
    process.env.OPENAEON_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPENAEON_PROFILE);
