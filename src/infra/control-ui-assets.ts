...
import { resolveOPENAEONPackageRoot, resolveOPENAEONPackageRootSync } from "./openaeon-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveOPENAEONPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "openaeon" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveOPENAEONPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
  ...
  if (argv1Dir) {
    // openaeon.mjs or dist/<bundle>.js
