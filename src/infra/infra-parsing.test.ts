...
describe("infra parsing", () => {
  describe("diagnostic flags", () => {
    ...
    it("treats env true as wildcard", () => {
      const env = { OPENAEON_DIAGNOSTICS: "1" } as NodeJS.ProcessEnv;
      expect(isDiagnosticFlagEnabled("anything.here", undefined, env)).toBe(true);
    });

    it("treats env false as disabled", () => {
      const env = { OPENAEON_DIAGNOSTICS: "0" } as NodeJS.ProcessEnv;
  ...
  describe("isMainModule", () => {
    ...
    it("returns true for dist/entry.js when launched via openaeon.mjs wrapper", () => {
      expect(
        isMainModule({
          currentFile: "/repo/dist/entry.js",
          argv: ["node", "/repo/openaeon.mjs"],
          cwd: "/repo",
          env: {},
          wrapperEntryPairs: [{ wrapperBasename: "openaeon.mjs", entryBasename: "entry.js" }],
        }),
      ).toBe(true);
    });
    ...
    it("returns false when running under PM2 but this module is imported", () => {
      expect(
        isMainModule({
          currentFile: "/repo/node_modules/openaeon/dist/index.js",
