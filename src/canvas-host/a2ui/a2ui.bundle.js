...
var OPENAEONA2UIHost = class extends i$6 {
  ...
  `;
	connectedCallback() {
		...
		for (const eventName of ["openaeon:a2ui-action-status"]) globalThis.addEventListener(eventName, this.#statusListener);
		this.#syncSurfaces();
	}
	disconnectedCallback() {
		super.disconnectedCallback();
		if (this.#statusListener) {
			for (const eventName of ["openaeon:a2ui-action-status"]) globalThis.removeEventListener(eventName, this.#statusListener);
	...
	#handleA2UIAction(evt) {
		...
		const handler = globalThis.webkit?.messageHandlers?.openaeonCanvasA2UIAction ?? globalThis.openaeonCanvasA2UIAction;
		if (handler?.postMessage) try {
			if (handler === globalThis.openaeonCanvasA2UIAction) handler.postMessage(JSON.stringify({ userAction }));
...
if (!customElements.get("openaeon-a2ui-host")) customElements.define("openaeon-a2ui-host", OPENAEONA2UIHost);
