...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.openaeonCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.openaeonCanvasA2UIAction &&
        typeof window.openaeonCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.openaeonSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("openaeon:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android OPENAEON node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.openaeonSendUserAction === "function"
        ? window.openaeonSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OPENAEON_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.OPENAEON_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>OPENAEON Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
