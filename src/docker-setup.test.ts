...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    OPENAEON_GATEWAY_TOKEN: "test-token",
    OPENAEON_CONFIG_DIR: join(sandbox.rootDir, "config"),
    OPENAEON_WORKSPACE_DIR: join(sandbox.rootDir, "openclaw"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      OPENAEON_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      OPENAEON_EXTRA_MOUNTS: undefined,
      OPENAEON_HOME_VOLUME: "openclaw-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("OPENAEON_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("OPENAEON_EXTRA_MOUNTS=");
    expect(envFile).toContain("OPENAEON_HOME_VOLUME=openclaw-home");
    ...
    expect(log).toContain("--build-arg OPENAEON_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      OPENAEON_CONFIG_DIR: configDir,
      OPENAEON_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("reuses existing config token when OPENAEON_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    const result = runDockerSetup(activeSandbox, {
      OPENAEON_GATEWAY_TOKEN: undefined,
      OPENAEON_CONFIG_DIR: configDir,
      OPENAEON_WORKSPACE_DIR: workspaceDir,
    });
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("OPENAEON_GATEWAY_TOKEN=config-token-123");
  });

  it("rejects injected multiline OPENAEON_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      OPENAEON_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("OPENAEON_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid OPENAEON_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      OPENAEON_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid OPENAEON_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      OPENAEON_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("OPENAEON_HOME_VOLUME must match");
  });
