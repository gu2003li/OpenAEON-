...
import { resolveStateDir, type OPENAEONConfig } from "../config/config.js";
...
function collectAuthStorePaths(config: OPENAEONConfig, stateDir: string): string[] {
...
export async function runSecretsAudit(
  ...
  const previousAuthStoreReadOnly = process.env.OPENAEON_AUTH_STORE_READONLY;
  ...
  try {
    ...
    const config = snapshot.valid ? snapshot.config : ({} as OPENAEONConfig);
    ...
    if (previousAuthStoreReadOnly === undefined) {
      delete process.env.OPENAEON_AUTH_STORE_READONLY;
    } else {
      process.env.OPENAEON_AUTH_STORE_READONLY = previousAuthStoreReadOnly;
    }
  }
}
