...
function getSecretProviders(config: OPENAEONConfig): Record<string, SecretProviderConfig> {
...
function removeSecretProvider(config: OPENAEONConfig, providerAlias: string): boolean {
  if (!isRecord(config.secrets?.providers)) {
    return false;
  }
...
function buildCandidates(config: OPENAEONConfig): ConfigureCandidate[] {
...
function toSourceChoices(config: OPENAEONConfig): Array<{ value: SecretRefSource; label: string }> {
...
async function configureProvidersInteractive(config: OPENAEONConfig): Promise<void> {
  while (true) {
...
function collectProviderPlanChanges(params: { original: OPENAEONConfig; next: OPENAEONConfig }): {
...
export async function runSecretsConfigureInteractive(
  ...
  if (!params.providersOnly) {
    ...
    if (candidates.length === 0) {
      throw new Error("No configurable secret-bearing fields found in openaeon.json.");
    }
  ...
  const plan: SecretsApplyPlan = {
    ...
    generatedBy: "openaeon secrets configure",
