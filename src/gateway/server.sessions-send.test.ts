...
const { createOpenClawTools } = await import("../agents/openaeon-tools.js");
...
beforeAll(async () => {
  envSnapshot = captureEnv(["OPENAEON_GATEWAY_PORT", "OPENAEON_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.OPENAEON_GATEWAY_PORT = String(gatewayPort);
  process.env.OPENAEON_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("OPENAEON_CONFIG_PATH missing in gateway test environment");
      }
