...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "OPENAEON_SKIP_BROWSER_CONTROL_SERVER",
  "OPENAEON_SKIP_GMAIL_WATCHER",
  "OPENAEON_SKIP_CANVAS_HOST",
  "OPENAEON_BUNDLED_PLUGINS_DIR",
  "OPENAEON_SKIP_CHANNELS",
  "OPENAEON_SKIP_PROVIDERS",
  "OPENAEON_SKIP_CRON",
  "OPENAEON_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openaeon-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.OPENAEON_STATE_DIR = path.join(tempHome, ".openaeon");
  delete process.env.OPENAEON_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.OPENAEON_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.OPENAEON_SKIP_GMAIL_WATCHER = "1";
  process.env.OPENAEON_SKIP_CANVAS_HOST = "1";
  process.env.OPENAEON_SKIP_CHANNELS = "1";
  process.env.OPENAEON_SKIP_PROVIDERS = "1";
  process.env.OPENAEON_SKIP_CRON = "1";
  process.env.OPENAEON_TEST_MINIMAL_GATEWAY = "1";
  process.env.OPENAEON_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "openaeon-test-no-bundled-extensions")
    : "openaeon-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "openaeon-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".openaeon-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__openaeonTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__openaeonTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["OPENAEON_GATEWAY_TOKEN"]);
  const prev = process.env.OPENAEON_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.OPENAEON_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.OPENAEON_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
