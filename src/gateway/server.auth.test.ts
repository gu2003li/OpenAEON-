...
describe("gateway server auth/connect", () => {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.OPENAEON_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.OPENAEON_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.OPENAEON_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.OPENAEON_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
      ...
      for (const testCase of [
        {
          env: {
            OPENAEON_VERSION: " ",
            OPENAEON_SERVICE_VERSION: "2.4.6-service",
  ...
  describe("token auth", () => {
    ...
    beforeAll(async () => {
      prevToken = process.env.OPENAEON_GATEWAY_TOKEN;
      process.env.OPENAEON_GATEWAY_TOKEN = "secret";
      port = await getFreePort();
      server = await startGatewayServer(port);
    });
