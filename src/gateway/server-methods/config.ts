...
import type { OPENAEONConfig } from "../../config/types.openaeon.js";
import {
...
import { loadOPENAEONPlugins } from "../../plugins/loader.js";
...
function parseValidateConfigFromRawOrRespond(
  ...
  snapshot: Awaited<ReturnType<typeof readConfigFileSnapshot>>,
  respond: RespondFn,
): { config: OPENAEONConfig; schema: ConfigSchemaResponse } | null {
...
function loadSchemaWithPlugins(): ConfigSchemaResponse {
  ...
  const pluginRegistry = loadOPENAEONPlugins({
    config: cfg,
    cache: true,
    workspaceDir,
    logger: {
  ...
  // our cache. However, both loadConfig() and loadOPENAEONPlugins() already cache
