...
import type { ModelsConfig, OPENAEONConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.OPENAEON_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.OPENAEON_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.OPENAEON_LIVE_GATEWAY_PROVIDERS);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.OPENAEON_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.OPENAEON_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<OPENAEONConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    stateDir: process.env.OPENAEON_STATE_DIR,
  };
  ...
  process.env.OPENAEON_AGENT_DIR = tempAgentDir;
  ...
  const sanitizedCfg: OPENAEONConfig = {
    ...params.cfg,
  ...
  try {
    ...
    process.env.OPENAEON_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.OPENAEON_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.OPENAEON_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via OPENAEON_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
