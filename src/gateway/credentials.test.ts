...
function cfg(input: Partial<OPENAEONConfig>): OPENAEONConfig {
  return input as OPENAEONConfig;
}

type ResolveFromConfigInput = Parameters<typeof resolveGatewayCredentialsFromConfig>[0];
type GatewayConfig = NonNullable<OPENAEONConfig["gateway"]>;
...
const DEFAULT_GATEWAY_ENV = {
  OPENAEON_GATEWAY_TOKEN: "env-token",
  OPENAEON_GATEWAY_PASSWORD: "env-password",
...
describe("resolveGatewayCredentialsFromConfig", () => {
  ...
  it("can disable legacy AEONPROPHET env fallback", () => {
    const resolved = resolveGatewayCredentialsFromConfig({
      cfg: cfg({
        gateway: {
          mode: "local",
      ...
      env: {
        ...
        AEONPROPHET_GATEWAY_PASSWORD: "legacy-password",
      } as NodeJS.ProcessEnv,
      includeLegacyEnv: false,
    });
