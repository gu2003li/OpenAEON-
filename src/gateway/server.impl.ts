...
import { ensureOPENAEONCliOnPath } from "../infra/path-env.js";
...
import { startEvolutionMonitor } from "./server-evolution.js";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.OPENAEON_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.OPENAEON_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "OPENAEON_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "OPENAEON_RAW_STREAM_PATH",
  ...
  if (configSnapshot.legacyIssues.length > 0) {
    ...
    if (!migrated) {
      throw new Error(
        `Legacy config entries detected but auto-migration failed. Run "${formatCliCommand("openaeon doctor")}" to migrate.`,
      );
    }
  ...
  if (configSnapshot.exists && !configSnapshot.valid) {
    ...
    throw new Error(
      `Invalid config at ${configSnapshot.path}.\n${issues}\nRun "${formatCliCommand("openaeon doctor")}" to repair, then retry.`,
    );
  }
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `openaeon config set gateway.auth.mode token` and `openaeon config set gateway.auth.token <token>`.",
      );
    }
  }
  cfgAtStart = (
  ...
  attachGatewayWsHandlers({
    ...
    context: {
      ...
      workspaceDir: defaultWorkspaceDir,
      agentDir: path.join(defaultWorkspaceDir, "agents", defaultAgentId),
    },
  });
  logGatewayStartup({
    cfg: cfgAtStart,
