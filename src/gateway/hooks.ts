...
export function resolveHooksConfig(cfg: OPENAEONConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: OPENAEONConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-openaeon-token"] === "string"
      ? req.headers["x-openaeon-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
