...
export function createAgentEventHandler({
  ...
  const emitChatDelta = (
    ...
    if (progress) {
      const now = Date.now();
      const last = lastProgressUpdateByRunId.get(clientRunId);
      // Throttle updates: at most once every 2 seconds, and only if content changed
      if (!last || (progress !== last.text && now - last.ts > 2000)) {
        lastProgressUpdateByRunId.set(clientRunId, { text: progress, ts: now });
        void callGateway({
          method: "sessions.patch",
          params: { key: sessionKey, subject: progress },
        }).catch(() => {}); // Best-effort
      }
    }
