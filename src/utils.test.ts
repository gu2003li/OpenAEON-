...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("openaeon-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("openaeon-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("openaeon-lid-a-", (first) => {
      withTempDirSync("openaeon-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.openaeon when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "openaeon-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers OPENAEON_HOME over HOME", () => {
    vi.stubEnv("OPENAEON_HOME", "/srv/openaeon-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/openaeon-home"));
...
describe("shortenHomePath", () => {
  it("uses $OPENAEON_HOME prefix when OPENAEON_HOME is set", () => {
    vi.stubEnv("OPENAEON_HOME", "/srv/openaeon-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/openaeon-home")}/.openaeon/openaeon.json`)).toBe(
      "$OPENAEON_HOME/.openaeon/openaeon.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $OPENAEON_HOME replacement when OPENAEON_HOME is set", () => {
    vi.stubEnv("OPENAEON_HOME", "/srv/openaeon-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/openaeon-home")}/.openaeon/openaeon.json`),
    ).toBe("config: $OPENAEON_HOME/.openaeon/openaeon.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/openaeon")).toBe(path.resolve(os.homedir(), "openaeon"));
  });
  ...
  it("prefers OPENAEON_HOME for tilde expansion", () => {
    vi.stubEnv("OPENAEON_HOME", "/srv/openaeon-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/openaeon")).toBe(path.resolve("/srv/openaeon-home", "openaeon"));
