...
describe("resolveGatewayConnection", () => {
  ...
  it("uses OPENAEON_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ OPENAEON_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers OPENAEON_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ OPENAEON_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
