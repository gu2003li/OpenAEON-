...
export async function loginQwenWeb(params: {
  ...
  const profile = resolveProfile(browserConfig, "openaeon");
  if (!profile) {
    throw new Error(`Could not resolve browser profile 'openaeon'`);
  }

  params.onProgress("Launching browser...");
  const qwenUrl = "https://www.qianwen.com/";
  const running = await launchOPENAEONChrome(browserConfig, profile, { initialUrl: qwenUrl });
  ...
  try {
    ...
    await stopOPENAEONChrome(running);
  }
}
