...
function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    openclaw: process.env.OPENAEON_STATE_DIR,
    legacy: process.env.AEONPROPHET_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.OPENAEON_STATE_DIR).toBe(snapshot.openclaw);
  expect(process.env.AEONPROPHET_STATE_DIR).toBe(snapshot.legacy);
}
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips OPENAEON_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/openclaw-state-dir-test");
    expect(process.env.OPENAEON_STATE_DIR).toBe("/tmp/openclaw-state-dir-test");
    expect(process.env.AEONPROPHET_STATE_DIR).toBeUndefined();
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("openclaw-state-dir-env-", async ({ tempRoot, stateDir }) => {
      ...
      expect(process.env.OPENAEON_STATE_DIR).toBe(stateDir);
      expect(process.env.AEONPROPHET_STATE_DIR).toBeUndefined();
  ...
  it("withStateDirEnv restores both env vars when legacy var was previously set", async () => {
    ...
    process.env.OPENAEON_STATE_DIR = "/tmp/original-openclaw";
    process.env.AEONPROPHET_STATE_DIR = "/tmp/original-legacy";
    const prev = snapshotCurrentStateDirVars();
