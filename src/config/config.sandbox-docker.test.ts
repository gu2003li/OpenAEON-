...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("openaeon-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "openaeon-sandbox-browser-global" },
      agentBrowser: { network: "openaeon-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("openaeon-sandbox-browser-agent");
  });
