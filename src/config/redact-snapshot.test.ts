...
import type { ConfigFileSnapshot } from "./types.openaeon.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.openaeon/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/openaeon/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/openaeon/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/openaeon/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/openaeon/nickserv-password.txt");
  ...
  it("contract-covers dynamic catchall/record paths for redact+restore", () => {
    const hints = mapSensitivePaths(OPENAEONSchema, "", {});
    const snapshot = makeSnapshot({
      env: {
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = OPENAEONSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "OPENAEONConfig";
    const hints = mapSensitivePaths(OPENAEONSchema, "", {});
