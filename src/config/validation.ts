...
import type { OPENAEONConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OPENAEONConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: OPENAEONConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: OPENAEONConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OPENAEONSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OPENAEONConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OPENAEONConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as OPENAEONConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as OPENAEONConfig,
  };
}
