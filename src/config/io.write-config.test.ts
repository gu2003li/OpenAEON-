...
describe("config io write", () => {
  ...
  async function writeConfigAndCreateIo(params: {
    ...
    const configPath = path.join(params.home, ".openaeon", "openaeon.json");
  ...
  async function writeGatewayPatchAndReadLastAuditEntry(params: {
    ...
    const auditPath = path.join(params.home, ".openaeon", "logs", "config-audit.jsonl");
  ...
  it("persists caller changes onto resolved config without leaking runtime defaults", async () => {
    await withTempHome("openaeon-config-io-", async (home) => {
  ...
  it('shows actionable guidance for dmPolicy="open" without wildcard allowFrom', async () => {
    await withTempHome("openaeon-config-io-", async (home) => {
      ...
      const invalidConfig: OPENAEONConfig = {
        channels: {
          telegram: {
            dmPolicy: "open",
            allowFrom: [],
      ...
      } satisfies OPENAEONConfig;

      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        "openaeon config set channels.telegram.allowFrom '[\"*\"]'",
      );
      await expect(io.writeConfigFile(invalidConfig)).rejects.toThrow(
        'openaeon config set channels.telegram.dmPolicy "pairing"',
      );
    });
  });
  ...
  it("does not mutate caller config when unsetPaths is applied on first write", async () => {
    await withTempHome("openaeon-config-io-", async (home) => {
      const configPath = path.join(home, ".openaeon", "openaeon.json");
      const io = createConfigIO({
        env: {} as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
  ...
  it("records gateway watch session markers in config audit entries", async () => {
    await withTempHome("openaeon-config-io-", async (home) => {
      const { last } = await writeGatewayPatchAndReadLastAuditEntry({
        ...
        env: {
          OPENAEON_WATCH_MODE: "1",
          OPENAEON_WATCH_SESSION: "watch-session-1",
          OPENAEON_WATCH_COMMAND: "gateway --force",
