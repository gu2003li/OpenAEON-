...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openaeon-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ OPENAEON_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-openaeon",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-openaeon#general");
  });
  ...
  it("derives session transcripts dir from OPENAEON_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { OPENAEON_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const stateDir = path.resolve("/home/user/.openaeon");
  ...
  it("resolves cross-agent paths when OPENAEON_STATE_DIR differs from stored paths", () => {
  ...
  it("resolveSessionFilePathOptions keeps explicit agentId alongside absolute store path", () => {
    const storePath = "/tmp/openaeon/agents/main/sessions/sessions.json";
  ...
  it("falls back to derived transcript path when sessionFile is outside agent sessions directories", () => {
    withStateDir(path.resolve("/home/user/.openaeon"), () => {
      const sessionFile = resolveSessionFilePath(
        "sess-1",
      ...
      expect(sessionFile).toBe(
        path.join(
          path.resolve("/home/user/.openaeon"),
          "agents",
          "bot1",
          "sessions",
          "sess-1.jsonl",
