...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    await withEnvAsync({ OPENAEON_BASE_ENV: "base" }, async () => {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.OPENAEON_BASE_ENV ?? "") + "|" + (process.env.OPENAEON_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { OPENAEON_TEST_ENV: "ok" },
        },
      );
