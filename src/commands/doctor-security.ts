...
import type { OPENAEONConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OPENAEONConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("openaeon security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.openaeon/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("openaeon approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("openaeon configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("openaeon config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("openaeon doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("openaeon config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('openaeon config set session.dmScope "per-channel-peer"') +
