...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under OPENAEON_AGENT_DIR when set", async () => {
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    ...
    process.env.OPENAEON_AGENT_DIR = kidAgentDir;
  ...
  it("syncs siblings from explicit agentDir outside OPENAEON_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-oauth-external-"));
    process.env.OPENAEON_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* OPENAEON_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPENAEON_AGENT_DIR when set", async () => {
