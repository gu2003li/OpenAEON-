...
function prependInternalEventContext(
  ...
  if (body.includes("OPENAEON runtime context (internal):")) {
    return body;
  }
...
export async function agentCommand(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("openaeon agents list")}" to see configured agents.`,
      );
    }
  }
