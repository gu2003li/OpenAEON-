...
function pruneAuthProfiles(
  cfg: OPENAEONConfig,
  profileIds: Set<string>,
): { next: OPENAEONConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("openaeon models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("openaeon models auth setup-token")} or ${formatCliCommand(
      "openaeon configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "openaeon models auth login --provider openai-codex",
    )} or ${formatCliCommand("openaeon configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("openaeon configure")}\` or \`${formatCliCommand("openaeon onboard")}\`.`;
}
