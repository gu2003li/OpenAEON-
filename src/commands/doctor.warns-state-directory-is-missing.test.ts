...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    process.env.OPENAEON_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when OPENAEON_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.OPENAEON_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
