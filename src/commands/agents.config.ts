...
function resolveAgentName(cfg: OPENAEONConfig, agentId: string) {
...
function resolveAgentModel(cfg: OPENAEONConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OPENAEONConfig): AgentSummary[] {
