...
const SEARCH_TOOL = "https://docs.openaeon.ai/mcp.SearchOPENAEON";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.openaeon.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('openaeon docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.openaeon.ai/");
      runtime.log(`Search: ${formatCliCommand('openaeon docs "your query"')}`);
    }
    return;
  }
