...
beforeAll(() => {
  envSnapshot = captureEnv(["OPENAEON_PROFILE"]);
  process.env.OPENAEON_PROFILE = "isolated";
});
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.openaeon.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.openaeon.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.OPENAEON_GATEWAY_TOKEN = "abcd1234";
    try {
