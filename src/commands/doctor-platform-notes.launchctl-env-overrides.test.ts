...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "OPENAEON_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("OPENAEON_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv OPENAEON_GATEWAY_TOKEN");
    expect(message).not.toContain("OPENAEON_GATEWAY_PASSWORD");
  });
