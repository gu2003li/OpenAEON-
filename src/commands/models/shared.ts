...
export async function loadValidConfigOrThrow(): Promise<OPENAEONConfig> {
...
export async function updateConfig(
  mutator: (cfg: OPENAEONConfig) => OPENAEONConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OPENAEONConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OPENAEONConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("openaeon agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
