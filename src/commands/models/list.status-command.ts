...
import { resolveOPENAEONAgentDir } from "../../agents/agent-paths.js";
import {
  resolveAgentDir,
...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOPENAEONAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("openaeon models auth setup-token")}\` or \`${formatCliCommand("openaeon configure")}\`.`
          : `Run \`${formatCliCommand("openaeon configure")}\` or set an API key env var.`;
