...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: OPENAEONConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: OPENAEONConfig) => { changed: boolean; next: OPENAEONConfig };
  defaultModel: string;
  overrideConfig: OPENAEONConfig;
  alreadyDefaultConfig: OPENAEONConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: OPENAEONConfig) => config,
      applyDefaultConfig: (config: OPENAEONConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: OPENAEONConfig = { agents: { defaults: {} } };
