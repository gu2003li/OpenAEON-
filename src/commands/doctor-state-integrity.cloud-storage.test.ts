...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("anchors cloud detection to OS homedir when OPENAEON_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".openclaw");
    const originalOpenClawHome = process.env.OPENAEON_HOME;
    process.env.OPENAEON_HOME = "/tmp/openclaw-home-override";
    ...
    try {
      ...
      if (originalOpenClawHome === undefined) {
        ...
        process.env.OPENAEON_HOME = originalOpenClawHome;
      }
    }
  });
