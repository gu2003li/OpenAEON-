...
export function normalizeCompatibilityConfigValues(cfg: OPENAEONConfig): {
  config: OPENAEONConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: OPENAEONConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as OPENAEONConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as OPENAEONConfig["browser"],
    };
    changes.push(
