...
async function noteChannelPrimer(
  ...
  await prompter.note(
    [
      ...
      `批准配对命令: ${formatCliCommand("openaeon pairing approve <channel> <code>")}`,
  ...
  const selectPolicy = async (policy: ChannelOnboardingDmPolicy) => {
    await prompter.note(
      [
        ...
        `Approve: ${formatCliCommand(`openaeon pairing approve ${policy.channel} <code>`)}`,
    ...
    if (!getChannelPlugin(channel)) {
      ...
      if (adapter) {
        await prompter.note(
          ...
          )}\` and \`${formatCliCommand("openaeon plugins enable " + channel)}\`, then restart the gateway.`,
          "Channel setup",
        );
    ...
    const choice = (await prompter.select({
      ...
      options: [
        ...buildSelectionOptions(entries),
        {
          ...
          hint: `稍后可通过 \`${formatCliCommand("openaeon channels add")}\` 继续添加`,
        },
      ],
