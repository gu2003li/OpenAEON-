...
type EnvSnapshot = {
  HOME?: string;
  OPENAEON_HOME?: string;
  OPENAEON_STATE_DIR?: string;
  OPENAEON_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    OPENAEON_HOME: process.env.OPENAEON_HOME,
    OPENAEON_STATE_DIR: process.env.OPENAEON_STATE_DIR,
    OPENAEON_OAUTH_DIR: process.env.OPENAEON_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: OPENAEONConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: OPENAEONConfig) {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    process.env.OPENAEON_HOME = tempHome;
    process.env.OPENAEON_STATE_DIR = path.join(tempHome, ".openclaw");
    delete process.env.OPENAEON_OAUTH_DIR;
    fs.mkdirSync(process.env.OPENAEON_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when OPENAEON_OAUTH_DIR is explicitly configured", async () => {
    process.env.OPENAEON_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: OPENAEONConfig = {};
    const confirmSkipInNonInteractive = await runStateIntegrity(cfg);
    expect(confirmSkipInNonInteractive).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
