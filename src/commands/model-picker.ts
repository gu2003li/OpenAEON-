...
type PromptDefaultModelResult = { model?: string; config?: OPENAEONConfig };
...
function resolveConfiguredModelRaw(cfg: OPENAEONConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: OPENAEONConfig): string[] {
...
export function applyPrimaryModel(cfg: OPENAEONConfig, model: string): OPENAEONConfig {
...
export function applyModelAllowlist(cfg: OPENAEONConfig, models: string[]): OPENAEONConfig {
  const defaults = cfg.agents?.defaults;
