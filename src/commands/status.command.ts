...
export async function statusCommand(
  ...
  runtime.log(theme.heading("OPENAEON status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`openaeon devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("openaeon devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("openaeon devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("openaeon security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("openaeon security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.openaeon.ai/faq");
  runtime.log("Troubleshooting: https://docs.openaeon.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("openaeon status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("openaeon logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("openaeon status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("openaeon gateway probe")}`);
  }
}
