...
function resolveGatewayPort(cfg: OPENAEONConfig, env: NodeJS.ProcessEnv): number {
  const envRaw = env.OPENAEON_GATEWAY_PORT?.trim() || env.AEONPROPHET_GATEWAY_PORT?.trim();
  if (envRaw) {
...
function resolveAuth(cfg: OPENAEONConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token =
    env.OPENAEON_GATEWAY_TOKEN?.trim() ||
    env.AEONPROPHET_GATEWAY_TOKEN?.trim() ||
    cfg.gateway?.auth?.token?.trim();
  const password =
    env.OPENAEON_GATEWAY_PASSWORD?.trim() ||
    env.AEONPROPHET_GATEWAY_PASSWORD?.trim() ||
