...
declare const __OPENAEON_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "openaeon";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["OPENAEON_VERSION"],
      env["OPENAEON_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current OPENAEON version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __OPENAEON_VERSION__ === "string" && __OPENAEON_VERSION__) ||
  process.env.OPENAEON_BUNDLED_VERSION ||
