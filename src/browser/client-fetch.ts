...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-openaeon-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-openaeon-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-openaeon-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the OPENAEON gateway (OPENAEON.app menubar, or \`${formatCliCommand("openaeon gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the OPENAEON browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the OPENAEON browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
