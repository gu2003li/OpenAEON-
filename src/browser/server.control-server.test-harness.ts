...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.OPENAEON_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/openaeon" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "openaeon",
        profiles: {
          openaeon: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchOPENAEONChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.OPENAEON_GATEWAY_PORT;
  process.env.OPENAEON_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.OPENAEON_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.OPENAEON_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.OPENAEON_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.OPENAEON_GATEWAY_PASSWORD;
  } else {
    process.env.OPENAEON_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
