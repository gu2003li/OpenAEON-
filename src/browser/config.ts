import type { BrowserConfig, BrowserProfileConfig, OPENAEONConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OPENAEON_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "openaeon" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_OPENAEON_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "openaeon" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OPENAEON_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OPENAEON_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an OPENAEON browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for openaeon/openaeon CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OPENAEONConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPENAEON_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "openaeon";
