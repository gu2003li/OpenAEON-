...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "OPENAEON_EXTENSION_RELAY_RECONNECT_GRACE_MS",
      "OPENAEON_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS",
    ]);
    process.env.OPENAEON_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
    delete process.env.OPENAEON_EXTENSION_RELAY_RECONNECT_GRACE_MS;
    delete process.env.OPENAEON_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS;
  });
  ...
  it("closes CDP clients after reconnect grace when extension stays disconnected", async () => {
    process.env.OPENAEON_EXTENSION_RELAY_RECONNECT_GRACE_MS = "150";
