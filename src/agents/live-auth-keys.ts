...
const GOOGLE_LIVE_SINGLE_KEY = "OPENAEON_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "OPENAEON_LIVE_ANTHROPIC_KEY",
    listVar: "OPENAEON_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "OPENAEON_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `OPENAEON_LIVE_${base}_KEY`;
