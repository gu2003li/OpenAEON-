...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "openaeon.sandbox=1");
  args.push("--label", `openaeon.sessionKey=${params.scopeKey}`);
  args.push("--label", `openaeon.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `openaeon.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "openaeon.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`openaeon sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`openaeon sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("openaeon sandbox recreate --all");
}
