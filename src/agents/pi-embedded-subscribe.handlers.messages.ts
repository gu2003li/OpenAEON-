...
import { runSubagentProgressAnnounceFlow } from "./subagent-announce.js";
...
export function handleMessageUpdate(
  ...
  // Detect and proactively announce <execution_progress> updates to parent agents.
  const latestProgress = extractLatestProgress(ctx.state.deltaBuffer);
  if (latestProgress && latestProgress !== ctx.state.lastAnnouncedProgress) {
    ctx.state.lastAnnouncedProgress = latestProgress;
    void runSubagentProgressAnnounceFlow({
      childSessionKey: (ctx.params.session as { id?: string }).id ?? ctx.params.sessionKey ?? "",
      runId: ctx.params.runId,
