...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.OPENAEON_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".openaeon", `workspace-${profile}`);
  }
  return path.join(home, ".openaeon", "workspace");
}
...
export const DEFAULT_LOGIC_GATES_FILENAME = "LOGIC_GATES.md";
const WORKSPACE_STATE_DIRNAME = ".openaeon";
...
export type WorkspaceBootstrapFileName =
  ...
  | typeof DEFAULT_LOGIC_GATES_FILENAME;
...
export async function ensureAgentWorkspace(params?: {
  ...
  const logicGatesPath = path.join(dir, DEFAULT_LOGIC_GATES_FILENAME);
  ...
  const logicGatesTemplate = await loadTemplate(DEFAULT_LOGIC_GATES_FILENAME);
  ...
  await writeFileIfMissing(logicGatesPath, logicGatesTemplate);
