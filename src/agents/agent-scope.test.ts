...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: OPENAEONConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: OPENAEONConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: OPENAEONConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: OPENAEONConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: OPENAEONConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: OPENAEONConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: OPENAEONConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("uses OPENAEON_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "openclaw-home");
    vi.stubEnv("OPENAEON_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as OPENAEONConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".openclaw", "workspace"));
  });

  it("uses OPENAEON_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "openclaw-home");
    vi.stubEnv("OPENAEON_HOME", home);
    // Clear state dir so it falls back to OPENAEON_HOME
    vi.stubEnv("OPENAEON_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as OPENAEONConfig, "main");
