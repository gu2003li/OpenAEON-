...
describe("buildAgentSystemPrompt", () => {
  ...
  it("guides subagent workflows to avoid polling loops", () => {
    ...
    expect(prompt).toContain(
      "If a task is complex, requires web research, or involves tedious coding/repetitive tasks, ALWAYS delegate it by spawning a sub-agent via `sessions_spawn`.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("delegated by a Supervisor (main agent)");
  ...
  it("renders depth-2 leaf guidance with parent orchestrator labels", () => {
    ...
    expect(prompt).toContain("delegated by a Supervisor (parent orchestrator)");
  ...
  it("omits spawning guidance for depth-1 leaf agents", () => {
    ...
    for (const testCase of leafCases) {
      ...
      if (testCase.name === "explicit maxSpawnDepth 1") {
        ...
        expect(prompt, testCase.name).toContain("You CAN spawn");
      }
      if (testCase.expectMainAgentLabel) {
        expect(prompt, testCase.name).toContain("delegated by a Supervisor (main agent)");
      }
    }
  });
});
