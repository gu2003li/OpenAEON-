...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): OPENAEONConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as OPENAEONConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): OPENAEONConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as OPENAEONConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: OPENAEONConfig, accountId = "media") {
    await handleTelegramAction(
