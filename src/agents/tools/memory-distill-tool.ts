 ...
 */
export async function distillMemory(options: { agentId?: string; workspaceDir?: string } = {}): Promise<DistillationResult> {
  ...
  try {
    ...
    const cognitiveEntropy = system ? (Math.min(100, 10 + Math.floor((stats.size / 51200) * 100 / 4) + 5)) : 15;
    ...
    const allAxiomsData = [...gateLines, ...newGatesWithMetadata].map(line => {
      ...
      let meta = { id: "unknown", weight: 1, peano: { x: 0.5 }, ts: now, ref: null as string|null };
      if (match) {
