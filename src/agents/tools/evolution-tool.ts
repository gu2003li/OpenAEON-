...
export function createEvolutionTool(opts: { workspaceDir?: string } = {}): AnyAgentTool {
  return {
    ...
    name: "evolution",
    description: "Deep reflection and synthesis tool for autonomous self-evolution (Z \u21CC Z\u00B2 + C).",
    ...
    execute: async (_id, args) => {
      ...
      const workspaceRoot = resolveWorkspaceRoot(opts.workspaceDir ?? cfg.agents?.defaults?.workspace);
        ...
        if (action === "reflect") {
          ...
          async function scanDir(dir: string) {
            ...
            for (const entry of entries) {
              ...
              if (entry.isDirectory()) {
                ...
                if (matches) {
                  todoMatches.push(...matches.map(m => `[${path.relative(workspaceRoot, fullPath)}] ${m.trim()}`));
                }
              }
            }
          }
          ...
          for (const claim of claims) {
            if (readmeContent.includes(claim.term)) {
              ...
              if (!exists) {
                divergences.push(` README claims '${claim.term}' but implementation file '${claim.path}' is missing or moved.`);
              }
            }
          }

          // 3. Read EVOLUTION.md for guidance
          const evolution = await fs.readFile(evolutionPath, "utf-8").catch(() => "No EVOLUTION.md found.");
          const gates = await fs.readFile(logicGatesPath, "utf-8").catch(() => "");
          const axioms = gates.split("\n").filter(l => l.trim().length > 0);
          
          const insight = divergences.length > 0 
