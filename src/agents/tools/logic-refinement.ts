...
const LogicRefinementSchema = Type.Object({
  action: Type.Enum({
    audit: "audit",
    prune: "prune",
  }, { description: "Action to perform: audit (check for inconsistency), prune (remove stale/contradictory gates)" }),
  peanoRange: Type.Optional(Type.Array(Type.Number(), { minItems: 2, maxItems: 2, description: "Optional [min, max] range for Peano x-coordinate (0-1) to audit a local cluster." })),
});
...
export function createLogicRefinementTool(): AgentTool {
  return {
    ...
    execute: async (_toolCallId: string, args: any) => {
      const { action, peanoRange } = args as { action: "audit" | "prune", peanoRange?: [number, number] };
      ...
      try {
        ...
        if (action === "audit") {
          ...
          const healthScore = totalAxioms === 0 ? 1.0 : Math.max(0, 1 - (contradictions.length * 0.2) - (redundancies.length * 0.1));
          const healthLabel = healthScore > 0.9 ? "High-Civi (Convergence)" : healthScore > 0.6 ? "Stable" : "Divergent (Chaos)";
          ...
          return jsonResult({
            ...
            text: (reports.concat(contradictions, redundancies).join("\n") || "Audit complete: Logic gates are in optimal alignment.") + 
