...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    openclaw: process.env.OPENAEON_GATEWAY_TOKEN,
    clawdbot: process.env.AEONPROPHET_GATEWAY_TOKEN,
  };
  ...
  beforeEach(() => {
    ...
    delete process.env.AEONPROPHET_GATEWAY_TOKEN;
  });
  ...
  afterAll(() => {
    if (envSnapshot.openclaw === undefined) {
      ...
      process.env.OPENAEON_GATEWAY_TOKEN = envSnapshot.openclaw;
    }
    if (envSnapshot.clawdbot === undefined) {
      delete process.env.AEONPROPHET_GATEWAY_TOKEN;
    } else {
      process.env.AEONPROPHET_GATEWAY_TOKEN = envSnapshot.clawdbot;
    }
  });
  ...
  it("uses OPENAEON_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.OPENAEON_GATEWAY_TOKEN = "local-env-token";
    process.env.AEONPROPHET_GATEWAY_TOKEN = "legacy-env-token";
    configState.value = {
      gateway: {
