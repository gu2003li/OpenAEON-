 ...
 * A common mistake — caused by the similarity with the `openaeon.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (openaeon.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.OPENAEON_AUTH_STORE_READONLY === "1";
