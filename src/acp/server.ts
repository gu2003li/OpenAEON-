...
function printHelp(): void {
  console.log(`Usage: openaeon acp [options]
...
if (isMainModule({ currentFile: fileURLToPath(import.meta.url) })) {
  ...
  if (argv.includes("--token") || argv.includes("--gateway-token")) {
    console.error(
      "Warning: --token can be exposed via process listings. Prefer --token-file or OPENAEON_GATEWAY_TOKEN.",
    );
  }
  if (argv.includes("--password") || argv.includes("--gateway-password")) {
    console.error(
      "Warning: --password can be exposed via process listings. Prefer --password-file or OPENAEON_GATEWAY_PASSWORD.",
    );
  }
