...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: OPENAEONConfig, policy: ChannelAccessPolicy) => OPENAEONConfig;
  resolveAllowlist: (params: { cfg: OPENAEONConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: OPENAEONConfig; resolved: TResolved }) => OPENAEONConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: OPENAEONConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: OPENAEONConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: OPENAEONConfig, policy: ChannelAccessPolicy): OPENAEONConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: OPENAEONConfig, policy: ChannelAccessPolicy): OPENAEONConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: OPENAEONConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: OPENAEONConfig; resolved: string[] }) => {
      calls.push("apply");
