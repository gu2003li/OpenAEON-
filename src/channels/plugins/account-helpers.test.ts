...
function cfg(accounts?: Record<string, unknown> | null): OPENAEONConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as OPENAEONConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as OPENAEONConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as OPENAEONConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as OPENAEONConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as OPENAEONConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as OPENAEONConfig)).toBe("default");
    });
  });
});
