...
function telegramCfg(): OPENAEONConfig {
  return { channels: { telegram: { botToken: "tok" } } } as OPENAEONConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: OPENAEONConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as OPENAEONConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as OPENAEONConfig;
...
function createSignalAccountOverrideCfg(): OPENAEONConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as OPENAEONConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as OPENAEONConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as OPENAEONConfig,
        accountId: undefined,
        params: {
