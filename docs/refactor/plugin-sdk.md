...
- Publish as `openaeon/plugin-sdk` (or export from core under `openaeon/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `OpenAEONPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OpenAEONConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OpenAEONConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OpenAEONConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OpenAEONConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OpenAEONConfig): string;
  };
};
```
...
- Introduce `openaeon/plugin-sdk`.
- Add `api.runtime` to `OpenAEONPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `openaeonRuntime: ">=2026.2.0"`).
