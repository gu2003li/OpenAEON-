---
summary: "OpenAEON on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up OpenAEON on DigitalOcean
  - Looking for cheap VPS hosting for OpenAEON
title: "DigitalOcean"
---

# OpenAEON on DigitalOcean

## Goal

Run a persistent OpenAEON Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status openaeon-gateway.service

# View logs
journalctl --user -u openaeon-gateway.service -f
```
...
```bash
openaeon channels login whatsapp
# Scan QR code
```
...
- `~/.openaeon/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u openaeon --no-pager -n 50
```
