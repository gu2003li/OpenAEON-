---
summary: "OpenAEON on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OpenAEON on a Raspberry Pi
  - Running OpenAEON on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# OpenAEON on Raspberry Pi

## Goal

Run a persistent, always-on OpenAEON Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
openaeon config set gateway.bind tailnet
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/openaeon-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
export NODE_COMPILE_CACHE=/var/tmp/openaeon-compile-cache
mkdir -p /var/tmp/openaeon-compile-cache
export OPENAEON_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `OPENAEON_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running OpenAEON, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit openaeon
```

```ini
[Service]
Environment=OPENAEON_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/openaeon-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep OpenAEON state/cache on SSD-backed storage to avoid SD-card
...
Most OpenAEON features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled openaeon

# Enable if not
sudo systemctl enable openaeon

# Start on boot
sudo systemctl start openaeon
```
...
```bash
# Check logs
journalctl -u openaeon --no-pager -n 100

# Common fix: rebuild
cd ~/openaeon  # if using hackable install
npm run build
sudo systemctl restart openaeon
```
