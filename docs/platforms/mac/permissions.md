...
- Same path: run the app from a fixed location (for OpenAEON, `dist/OpenAEON.app`).
...
```bash
sudo tccutil reset Accessibility ai.openaeon.mac
sudo tccutil reset ScreenCapture ai.openaeon.mac
...
Workaround: move files into the OpenAEON workspace (`~/.openaeon/workspace`) if you want to avoid per-folder grants.
