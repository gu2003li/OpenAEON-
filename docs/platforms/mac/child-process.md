...
service via the external `openaeon` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.openaeon.gateway`
  (or `ai.openaeon.<profile>` when using `--profile`/`OPENAEON_PROFILE`; legacy `com.openaeon.*` is supported).
