...
```bash
openaeon tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `openaeon tui --deliver`
...
- Check the Gateway logs: `openaeon logs --follow`.
- Confirm the agent can run: `openaeon status` and `openaeon models status`.
...
- No agents in picker: check `openaeon agents list` and your routing config.
