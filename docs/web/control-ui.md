...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/openaeon`)
...
If the page fails to load, start the Gateway first: `openaeon gateway`.
...
you revoke it with `openaeon devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.openaeon/openaeon.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop openaeon`, `please stop`) to abort out-of-band
