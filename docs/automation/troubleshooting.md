...
openaeon system heartbeat last
openaeon logs --follow
openaeon config get agents.defaults.heartbeat
openaeon channels status --probe
```
...
```bash
openaeon config get agents.defaults.heartbeat.activeHours
openaeon config get agents.defaults.heartbeat.activeHours.timezone
openaeon config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
openaeon cron list
openaeon logs --follow
```
