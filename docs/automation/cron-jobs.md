...
- Jobs persist under `~/.openaeon/cron/` so restarts don’t lose schedules.
...
openaeon cron list
openaeon cron run <job-id>
openaeon cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.openaeon/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `openaeon cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, OpenAEON applies a
...
If `delivery` is omitted for isolated jobs, OpenAEON defaults to `announce`.
...
- Job store: `~/.openaeon/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.openaeon/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, OpenAEON classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
- For removed run sessions no longer referenced by the session store, OpenAEON archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `openaeon cron runs` and adjust retention before logs become large
...
openaeon cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
openaeon cron edit <jobId> --agent ops
openaeon cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
openaeon cron run <jobId>
openaeon cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
openaeon cron edit <jobId> \
...
```bash
openaeon cron edit <jobId> --exact
```
...
```bash
openaeon system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`openaeon system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `OPENAEON_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- OpenAEON applies exponential retry backoff for recurring jobs after consecutive errors:
