...
read_when:
  - Running OpenAEON behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of OpenAEON
...
- You run OpenAEON behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. OpenAEON checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. OpenAEON extracts the user identity from the configured header
...
- OpenAEON can stay on loopback HTTP behind the proxy.
...
If OpenAEON itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://openaeon.example.com
    to: http://openaeon-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
openaeon.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy openaeon:18789 {
...
location / {
    ...
    proxy_pass http://openaeon:18789;
...
`openaeon security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update OpenAEON config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `openaeon security audit` and review findings
