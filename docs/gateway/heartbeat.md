...
- During heartbeat runs, OpenAEON treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `openaeon sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, OpenAEON skips the heartbeat run entirely (no model call).
...
```bash
openaeon system event --text "Check for urgent follow-ups" --mode now
```
