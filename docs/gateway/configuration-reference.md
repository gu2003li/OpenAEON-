...
description: "Complete field-by-field reference for ~/.openaeon/openaeon.json"
summary: "Complete reference for every OpenAEON config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.openaeon/openaeon.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — OpenAEON uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `openaeon doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["openaeon-dm"] },
      guilds: {
        "123456789012345678": {
...
- OpenAEON additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `openaeon plugins install @openaeon/mattermost`.
...
OpenAEON spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `openaeon channels add` (or channel onboarding) while still on a single-account top-level channel config, OpenAEON moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `openaeon doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@openaeon"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `openaeon.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, OpenAEON auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, OpenAEON assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "openaeon-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.openaeon/sandboxes`
...
noVNC observer access uses VNC auth by default and OpenAEON emits a short-lived token URL that serves a local bootstrap page; noVNC password is passed via URL fragment (instead of URL query).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `openaeon-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.openaeon/workspace",
        agentDir: "~/.openaeon/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@openaeon"] },
        sandbox: { mode: "off" },
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, OpenAEON starts a fresh thread session instead of inheriting parent transcript history.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:openaeon`   | All built-in tools (excludes provider plugins)                                           |
...
OpenAEON uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.openaeon/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `OPENAEON_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `openaeon onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `openaeon onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `openaeon onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `openaeon onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `openaeon onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `openaeon onboard --auth-choice minimax-api`.
...
- Loaded from `~/.openaeon/extensions`, `<workspace>/.openaeon/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `openaeon plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or OPENAEON_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `OPENAEON_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.openaeon-dev` + port `19001`), `--profile <name>` (uses `~/.openaeon-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.openaeon/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-openaeon-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `OPENAEON_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or OPENAEON_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__openaeon__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__openaeon__/a2ui/`
...
- Also serves A2UI at `/__openaeon__/a2ui/`.
...
- Hostname defaults to `openaeon`. Override with `OPENAEON_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.openaeon/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `openaeon dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.openaeon/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${OPENAEON_GATEWAY_TOKEN}" },
  },
}
```
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.openaeon/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/openaeon-vault-resolver",
...
- Legacy OAuth imports from `~/.openaeon/credentials/oauth.json`.
...
- Default log file: `/tmp/openaeon/openaeon-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `openaeon doctor --fix` can strip unknown keys).
