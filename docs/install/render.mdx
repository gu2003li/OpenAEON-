...
Deploy OpenAEON on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/openaeon/openaeon)
...
repository configures everything needed to run OpenAEON:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: OPENAEON_STATE_DIR
        value: /data/.openaeon
      - key: OPENAEON_WORKSPACE_DIR
        value: /data/workspace
      - key: OPENAEON_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/openaeon`.
...
If you use the original OpenAEON repository, Render will not auto-deploy your OpenAEON. To update it, run a manual Blueprint sync from the dashboard.
...
For OpenAEON, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any OpenAEON host.
