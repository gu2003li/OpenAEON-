...
Deploy OpenAEON on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/openaeon` — Control UI
...
- `OPENAEON_STATE_DIR=/data/.openaeon` (recommended)
- `OPENAEON_WORKSPACE_DIR=/data/workspace` (recommended)
- `OPENAEON_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your OpenAEON state + workspace so you can migrate to another host without losing config or memory.
