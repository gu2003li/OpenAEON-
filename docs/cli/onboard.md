---
summary: "CLI reference for `openaeon onboard` (interactive onboarding wizard)"
read_when:
...
```bash
openaeon onboard
openaeon onboard --flow quickstart
openaeon onboard --flow manual
openaeon onboard --mode remote --remote-url ws://gateway-host:18789
```
...
- Fastest first chat: `openaeon dashboard` (Control UI, no channel setup).
