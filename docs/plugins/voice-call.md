...
read_when:
  - You want to place an outbound voice call from OpenAEON
...
Voice calls for OpenAEON via a plugin. Supports outbound notifications and
...
- Use `openaeon voicecall ...` or the `voice_call` tool
...
```bash
openaeon plugins install ./extensions/voice-call
