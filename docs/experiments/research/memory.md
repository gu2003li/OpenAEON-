...
read_when:
  - Designing workspace memory (~/.openaeon/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep OpenAEON integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.openaeon/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.openaeon/workspace` as canonical human-readable memory.
...
```
~/.openaeon/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in OpenAEON**, but keep a separable core library.

### Why integrate into OpenAEON?

- OpenAEON already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `openaeon memory recall "…" --k 25 --since 30d`
  - `openaeon memory reflect --since 7d`
...
Pragmatic take for `~/.openaeon/workspace`:
