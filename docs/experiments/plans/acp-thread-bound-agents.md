...
This plan defines how OpenAEON should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- OpenAEON core owns ACP control plane concerns
...
OpenAEON should not reimplement ACP transport internals in core.
OpenAEON should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in OpenAEON, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under OpenAEON state dir
...
- ACP event stream is normalized to OpenAEON reply chunks
...
For the first production backend (`extensions/acpx`), OpenAEON and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses OpenAEON `sessionKey`)
...
- OpenAEON consumes ndjson events from `acpx --format json --json-strict`
