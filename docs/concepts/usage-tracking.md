...
- `/usage cost` in chats: local cost summary aggregated from OpenAEON session logs.
- CLI: `openaeon status --usage` prints a full per-provider breakdown.
- CLI: `openaeon channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
