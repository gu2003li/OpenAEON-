...
Tlon is a decentralized messenger built on Urbit. OpenAEON connects to your Urbit ship and can
...
```bash
openaeon plugins install @openaeon/tlon
```

Local checkout (when running from a git repo):

```bash
openaeon plugins install ./extensions/tlon
```
...
By default, OpenAEON blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `openaeon message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OpenAEON replies in-thread.
