...
read_when:
  - Setting up Twitch chat integration for OpenAEON
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OpenAEON connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
openaeon plugins install @openaeon/twitch
```

Local checkout (when running from a git repo):

```bash
openaeon plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `OPENAEON_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "openaeon", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use OPENAEON_TWITCH_ACCESS_TOKEN env var)
...
```bash
OPENAEON_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
