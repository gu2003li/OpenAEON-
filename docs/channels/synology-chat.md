---
summary: "Synology Chat webhook setup and OpenAEON config"
read_when:
  - Setting up Synology Chat with OpenAEON
...
```bash
openaeon plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your OpenAEON gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in OpenAEON.
...
- Pairing approvals work with:
  - `openaeon pairing list synology-chat`
  - `openaeon pairing approve synology-chat <CODE>`
...
```bash
openaeon message send --channel synology-chat --target 123456 --text "Hello from OpenAEON"
openaeon message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
