...
```bash
openaeon plugins install @openaeon/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
openaeon plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "OpenAEON" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `openaeon pairing list nextcloud-talk`
  - `openaeon pairing approve nextcloud-talk <CODE>`
