---
summary: "Mattermost bot setup and OpenAEON config"
read_when:
...
```bash
openaeon plugins install @openaeon/mattermost
```

Local checkout (when running from a git repo):

```bash
openaeon plugins install ./extensions/mattermost
```
...
4. Configure OpenAEON and start the gateway.
...
- Approve via:
  - `openaeon pairing list mattermost`
  - `openaeon pairing approve mattermost <CODE>`
...
Use these target formats with `openaeon message send` or cron/webhooks:
