...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects OpenAEON to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
openaeon plugins install @openaeon/feishu
```

Local checkout (when running from a git repo):

```bash
openaeon plugins install ./extensions/feishu
```
...
If you just installed OpenAEON, run the wizard:
...
2. Configuring app credentials in OpenAEON
...
1. You already ran `openaeon channels add` for Feishu
2. The gateway is running (`openaeon gateway status`)
...
```bash
openaeon pairing approve feishu <CODE>
```
...
2. Run `openaeon logs --follow` and look for `chat_id`
...
2. Run `openaeon logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `openaeon gateway status`  | Show gateway status           |
| `openaeon gateway install` | Install/start gateway service |
| `openaeon gateway stop`    | Stop gateway service          |
| `openaeon gateway restart` | Restart gateway service       |
| `openaeon logs --follow`   | Tail gateway logs             |
...
4. Check logs: `openaeon logs --follow`
...
5. Ensure the gateway is running: `openaeon gateway status`
6. Check logs: `openaeon logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.openaeon/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.openaeon/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
