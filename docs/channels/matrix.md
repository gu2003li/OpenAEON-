...
Matrix is an open, decentralized messaging protocol. OpenAEON connects as a Matrix **user**
...
```bash
openaeon plugins install @openaeon/matrix
```

Local checkout (when running from a git repo):

```bash
openaeon plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `openaeon plugins install @openaeon/matrix`
   - From a local checkout: `openaeon plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OpenAEON calls the same
     login endpoint, stores the access token in `~/.openaeon/credentials/matrix/credentials.json`,
...
- On first connection, OpenAEON requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  OpenAEON logs a warning.
...
`~/.openaeon/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `openaeon pairing list matrix`
  - `openaeon pairing approve matrix <CODE>`
...
- On startup, OpenAEON resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
