---
summary: "Use Synthetic's Anthropic-compatible API in OpenAEON"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. OpenAEON registers it as the
...
Note: OpenAEON's Anthropic client appends `/v1` to the base URL, so use
