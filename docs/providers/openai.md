---
summary: "Use OpenAI via API keys or Codex subscription in OpenAEON"
read_when:
  - You want to use OpenAI models in OpenAEON
...
```bash
openaeon onboard --auth-choice openai-api-key
# or non-interactive
openaeon onboard --openai-api-key "$OPENAI_API_KEY"
```
...
openaeon onboard --auth-choice openai-codex

# Or run OAuth directly
openaeon models auth login --provider openai-codex
```
...
OpenAEON uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), OpenAEON also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. OpenAEON enables it by default for
...
`baseUrl` on `api.openai.com`), OpenAEON now auto-enables OpenAI server-side
