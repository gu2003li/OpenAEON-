---
summary: "Use MiniMax M2.1 in OpenAEON"
read_when:
  - You want MiniMax models in OpenAEON
...
```bash
openaeon plugins enable minimax-portal-auth  # skip if already loaded.
openaeon gateway restart  # restart if gateway is already running
openaeon onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/openaeon/openaeon/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `openaeon.json`:
...
- Use `openaeon models list` and `openaeon models set minimax/MiniMax-M2.1` to switch.
...
- Running `openaeon configure` and selecting **MiniMax M2.1**, or
