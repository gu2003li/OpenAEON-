---
summary: "Use Venice AI privacy-focused models in OpenAEON"
read_when:
  - You want privacy-focused inference in OpenAEON
...
```bash
openaeon onboard --auth-choice venice-api-key
```
...
```bash
openaeon agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, OpenAEON shows all available Venice models. Pick based on your needs:
...
```bash
openaeon models set venice/claude-opus-45
openaeon models set venice/llama-3.3-70b
```

List all available models:

```bash
openaeon models list | grep venice
```

## Configure via `openaeon configure`

1. Run `openaeon configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
OpenAEON automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
openaeon agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
openaeon agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
openaeon agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
openaeon agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
openaeon agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `openaeon models list` to see currently available models. Some models may be temporarily offline.
