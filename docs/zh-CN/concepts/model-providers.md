...
- CLI 辅助工具：`openaeon onboard`、`openaeon models list`、`openaeon models set <provider/model>`。

## 内置提供商（pi-ai 目录）

OpenAEON 附带 pi-ai 目录。这些提供商**不需要** `models.providers` 配置；只需设置认证 + 选择模型。
...
- CLI：`openaeon onboard --auth-choice token`（粘贴 setup-token）或 `openaeon models auth paste-token --provider anthropic`
...
- CLI：`openaeon onboard --auth-choice openai-codex` 或 `openaeon models auth login --provider openai-codex`
...
- Antigravity OAuth 作为捆绑插件提供（`google-antigravity-auth`，默认禁用）。
  - 启用：`openaeon plugins enable google-antigravity-auth`
  - 登录：`openaeon models auth login --provider google-antigravity --set-default`
- Gemini CLI OAuth 作为捆绑插件提供（`google-gemini-cli-auth`，默认禁用）。
  - 启用：`openaeon plugins enable google-gemini-cli-auth`
  - 登录：`openaeon models auth login --provider google-gemini-cli --set-default`
  - 注意：你**不需要**将客户端 ID 或密钥粘贴到 `openaeon.json` 中。CLI 登录流程将令牌存储在 Gateway 网关主机的认证配置文件中。
...
openaeon models set opencode/claude-opus-4-5
