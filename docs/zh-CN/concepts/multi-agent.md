...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.openaeon/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.openaeon/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 状态目录：`~/.openaeon`（或 `OPENAEON_STATE_DIR`）
- 工作区：`~/.openaeon/workspace`（或 `~/.openaeon/workspace-<agentId>`）
- 智能体目录：`~/.openaeon/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.openaeon/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，OpenAEON 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.openaeon/workspace`（或当设置了 `OPENAEON_PROFILE` 时为 `~/.openaeon/workspace-<profile>`）。
- 状态默认为 `~/.openaeon/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.openaeon/credentials/whatsapp/personal
