...
summary: 在 exe.dev 上运行 OpenAEON Gateway 网关（VM + HTTPS 代理）以实现远程访问
title: exe.dev
x-i18n:
...
目标：OpenAEON Gateway 网关运行在 exe.dev VM 上，可从你的笔记本电脑通过以下地址访问：`https://<vm-name>.exe.xyz`
...
Shelley，[exe.dev](https://exe.dev) 的智能体，可以使用我们的提示立即安装 OpenAEON。使用的提示如下：
...
提示：保持此 VM **有状态**。OpenAEON 在 `~/.openaeon/` 和 `~/.openaeon/workspace/` 下存储状态。
...
## 4) 设置 nginx 将 OpenAEON 代理到端口 8000
...
访问 `https://<vm-name>.exe.xyz/?token=YOUR-TOKEN-FROM-TERMINAL`（参阅新手引导中的控制 UI 输出）。使用 `openaeon devices list` 和 `openaeon devices approve <requestId>` 批准设备。如有疑问，从浏览器使用 Shelley！
