...
通过一键模板在 Railway 上部署 OpenAEON，并在浏览器中完成设置。
...
- `https://<your-railway-domain>/openaeon` — 控制面板 UI
...
- `OPENAEON_STATE_DIR=/data/.openaeon`（推荐）
- `OPENAEON_WORKSPACE_DIR=/data/workspace`（推荐）
- `OPENAEON_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 OpenAEON 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
