...
LINE 通过 LINE Messaging API 连接到 OpenAEON。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
openaeon plugins install @openaeon/line
```

本地检出（从 git 仓库运行时）：

```bash
openaeon plugins install ./extensions/line
```
...
```bash
openaeon pairing list line
openaeon pairing approve line <CODE>
```
