...
- 通过 CLI 安装：`openaeon plugins install @openaeon/zalo`
...
1. 安装 Zalo 插件：
   - 从源代码检出：`openaeon plugins install ./extensions/zalo`
   - 从 npm（如果已发布）：`openaeon plugins install @openaeon/zalo`
...
- 使用聊天 id 作为目标。
- 示例：`openaeon message send --channel zalo --target 123456789 --message "hi"`。

## 故障排除

**Bot 不响应：**

- 检查 token 是否有效：`openaeon channels status --probe`
- 验证发送者已被批准（配对或 allowFrom）
- 检查 Gateway 网关日志：`openaeon logs --follow`
