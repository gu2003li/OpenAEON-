...
- `~/Library/Application Support/OpenAEON/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `openaeon-canvas://<session>/<path>`

示例：

- `openaeon-canvas://main/` → `<canvasRoot>/main/index.html`
- `openaeon-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `openaeon-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
openaeon nodes canvas present --node <id>
openaeon nodes canvas navigate --node <id> --url "/"
openaeon nodes canvas eval --node <id> --js "document.title"
openaeon nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__openaeon__/a2ui/
```
...
openaeon nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
openaeon nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "openaeon://agent?message=Review%20this%20design";
```
