...
你可以在 `~/.openaeon/openaeon.json` 中覆盖此设置：
...
所有日志配置都在 `~/.openaeon/openaeon.json` 的 `logging` 下。
...
- OpenAEON 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `openaeon plugins enable diagnostics-otel` 启用插件。
...
- `openaeon.tokens`（计数器，属性：`openaeon.token`、`openaeon.channel`、`openaeon.provider`、`openaeon.model`）
- `openaeon.cost.usd`（计数器，属性：`openaeon.channel`、`openaeon.provider`、`openaeon.model`）
- `openaeon.run.duration_ms`（直方图，属性：`openaeon.channel`、`openaeon.provider`、`openaeon.model`）
- `openaeon.context.tokens`（直方图，属性：`openaeon.context`、`openaeon.channel`、`openaeon.provider`、`openaeon.model`）

消息流：

- `openaeon.webhook.received`（计数器，属性：`openaeon.channel`、`openaeon.webhook`）
- `openaeon.webhook.error`（计数器，属性：`openaeon.channel`、`openaeon.webhook`）
- `openaeon.webhook.duration_ms`（直方图，属性：`openaeon.channel`、`openaeon.webhook`）
- `openaeon.message.queued`（计数器，属性：`openaeon.channel`、`openaeon.source`）
- `openaeon.message.processed`（计数器，属性：`openaeon.channel`、`openaeon.outcome`）
- `openaeon.message.duration_ms`（直方图，属性：`openaeon.channel`、`openaeon.outcome`）

队列 + 会话：

- `openaeon.queue.lane.enqueue`（计数器，属性：`openaeon.lane`）
- `openaeon.queue.lane.dequeue`（计数器，属性：`openaeon.lane`）
- `openaeon.queue.depth`（直方图，属性：`openaeon.lane` 或 `openaeon.channel=heartbeat`）
- `openaeon.queue.wait_ms`（直方图，属性：`openaeon.lane`）
- `openaeon.session.state`（计数器，属性：`openaeon.state`、`openaeon.reason`）
- `openaeon.session.stuck`（计数器，属性：`openaeon.state`）
- `openaeon.session.stuck_age_ms`（直方图，属性：`openaeon.state`）
- `openaeon.run.attempt`（计数器，属性：`openaeon.attempt`）
...
- `openaeon.webhook.error`
  - `openaeon.channel`、`openaeon.webhook`、`openaeon.chatId`、`openaeon.error`
- `openaeon.message.processed`
  - `openaeon.channel`、`openaeon.outcome`、`openaeon.chatId`、`openaeon.messageId`、`openaeon.sessionKey`、`openaeon.sessionId`、`openaeon.reason`
- `openaeon.session.stuck`
  - `openaeon.state`、`openaeon.ageMs`、`openaeon.queueDepth`、`openaeon.sessionKey`、`openaeon.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `openaeon doctor`。
