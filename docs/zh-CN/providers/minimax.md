---
read_when:
  - 你想在 OpenAEON 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 OpenAEON 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
openaeon plugins enable minimax-portal-auth  # 如果已加载则跳过
openaeon gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/openaeon/openaeon/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `openaeon models list` 和 `openaeon models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `openaeon configure` 并选择 **MiniMax M2.1**，或
