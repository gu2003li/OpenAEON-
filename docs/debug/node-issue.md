...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OpenAEON
...
Running OpenAEON via Node with `tsx` fails at startup with:

```
[openaeon] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `openaeon status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch openaeon.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node openaeon.mjs status` works on Node 25.
