...
OpenAEON integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- OpenAEON owns lifecycle when `channels.signal.autoStart=true`.
...
- OpenAEON spawns `imsg rpc` as a child process (legacy iMessage integration).
