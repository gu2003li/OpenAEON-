---
summary: "How OpenAEON builds prompt context and reports token usage + costs"
read_when:
...
OpenAEON tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

OpenAEON assembles its own system prompt on every run. It includes:
...
For images, OpenAEON downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from OpenAEON session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `openaeon status --usage` and `openaeon channels list` show
...
`cacheWrite`. If pricing is missing, OpenAEON shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. OpenAEON can
...
Anthropic's 1M context window is currently beta-gated. OpenAEON can inject the
...
OpenAEON skips the `context-1m-*` beta header because Anthropic currently
