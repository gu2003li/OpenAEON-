...
- With Anthropic API-key auth profiles, OpenAEON seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, OpenAEON injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
OpenAEON exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.openaeon/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$OPENAEON_STATE_DIR/logs/cache-trace.jsonl`
...
- `OPENAEON_CACHE_TRACE=1` enables cache tracing.
- `OPENAEON_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `OPENAEON_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `OPENAEON_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `OPENAEON_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
