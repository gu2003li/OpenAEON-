...
OpenAEON features that can generate provider usage or paid API calls.
...
- `openaeon status --usage` and `openaeon channels list` show provider **usage windows**
...
OpenAEON can pick up credentials from:
...
- `openaeon status --usage`
- `openaeon models status --json`
...
`openaeon models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
