...
- `gateway`/`node` approvals are controlled by `~/.openaeon/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  OpenAEON prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`openaeon security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `openaeon doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
