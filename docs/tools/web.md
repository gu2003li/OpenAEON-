...
OpenAEON ships two lightweight web tools:
...
If no `provider` is explicitly set, OpenAEON auto-detects which provider to use based on available API keys, checking in this order:
...
3. Run `openaeon configure --section web` to store the key in config (recommended), or set `BRAVE_API_KEY` in your environment.
...
**Recommended:** run `openaeon configure --section web`. It stores the key in
`~/.openaeon/openaeon.json` under `tools.web.search.apiKey`.

**Environment alternative:** set `BRAVE_API_KEY` in the Gateway process
environment. For a gateway install, put it in `~/.openaeon/.env` (or your
service environment). See [Env vars](/help/faq#how-does-openaeon-load-environment-variables).
...
environment. For a gateway install, put it in `~/.openaeon/.env`.

If no base URL is set, OpenAEON chooses a default based on the API key source:
...
For a gateway install, put it in `~/.openaeon/.env`.
