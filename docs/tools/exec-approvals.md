...
- **gateway host** → `openaeon` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.openaeon/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.openaeon/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `openaeon approvals allowlist ...`).
- `openaeon security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `openaeon doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.openaeon/exec-approvals.json` directly.

CLI: `openaeon approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
