---
summary: "Chrome extension: let OpenAEON drive your existing Chrome tab"
read_when:
...
The OpenAEON Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate openaeon-managed Chrome profile.
...
OpenAEON then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the OpenAEON release (npm package) as static files. There is no separate “build” step.

After upgrading OpenAEON:

- Re-run `openaeon browser extension install` to refresh the installed files under your OpenAEON state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

OpenAEON ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `OPENAEON_GATEWAY_TOKEN`)
...
- Open the tab you want OpenAEON to control.
...
- `ON`: attached; OpenAEON can drive that tab.
...
Debugging: `openaeon sandbox explain`
...
`openaeon browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `openaeon browser extension install` first to copy the extension to a stable location under your OpenAEON state directory.
...
- **This is not isolated** like the dedicated openaeon-managed profile.
