...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `openaeon sandbox explain`.
...
Legacy `agent.*` configs are migrated by `openaeon doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=openaeon-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENAEON_STATE_DIR:-$HOME/.openaeon}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
